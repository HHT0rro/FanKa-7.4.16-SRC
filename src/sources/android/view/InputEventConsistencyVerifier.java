...
public final class InputEventConsistencyVerifier {
    ...
    public void onTouchEvent(MotionEvent event, int nestingLevel) {
        ...
        try {
            ...
            int deviceId = event.getDeviceId();
            int source = event.getSource();
            if (!newStream && (i10 = this.mTouchEventStreamDeviceId) != -1 && (i10 != deviceId || this.mTouchEventStreamSource != source)) {
                    ...
                    default:
                        ...
                        if (actionMasked == 5) {
                            ...
                            if (actionIndex >= 0 && actionIndex < pointerCount) {
                                ...
                                int idBit = 1 << id2;
                                int i11 = this.mTouchEventStreamPointers;
                                if ((i11 & idBit) != 0) {
                                    problem("ACTION_POINTER_DOWN specified pointer id " + id2 + " which is already down.");
                                    this.mTouchEventStreamIsTainted = true;
                                } else {
                                    this.mTouchEventStreamPointers = i11 | idBit;
                                }
                                ensureHistorySizeIsZeroForThisAction(event);
                                break;
                            }
                            ...
                            if (actionIndex >= 0 && actionIndex < pointerCount) {
                                ...
                                int idBit2 = 1 << id3;
                                int i12 = this.mTouchEventStreamPointers;
                                if ((i12 & idBit2) == 0) {
                                    problem("ACTION_POINTER_UP specified pointer id " + id3 + " which is not currently down.");
                                    this.mTouchEventStreamIsTainted = true;
                                } else {
                                    this.mTouchEventStreamPointers = (~idBit2) & i12;
                                }
                                ensureHistorySizeIsZeroForThisAction(event);
                                break;
                            }
            ...
            if ((source & 2) != 0) {
                switch (action) {
                    ...
                    case 11:
                        ...
                        int i10 = this.mButtonsPressed | actionButton;
                        this.mButtonsPressed = i10;
                        if (actionButton == 32 && (buttonState & 2) != 0) {
                            this.mButtonsPressed = i10 | 2;
                        } else if (actionButton == 64 && (buttonState & 4) != 0) {
                            this.mButtonsPressed = i10 | 4;
                        }
                    ...
                    case 12:
                        ...
                        int i11 = this.mButtonsPressed & (~actionButton);
                        this.mButtonsPressed = i11;
                        if (actionButton == 32 && (buttonState & 2) == 0) {
                            this.mButtonsPressed = i11 & (-3);
                        } else if (actionButton == 64 && (buttonState & 4) == 0) {
                            this.mButtonsPressed = i11 & (-5);
                        }
                        if (this.mButtonsPressed != buttonState) {
                            problem(String.format("Reported button state differs from expected button state based on press and release events. Is 0x%08x but expected 0x%08x.", Integer.valueOf(buttonState), Integer.valueOf(this.mButtonsPressed)));
                            break;
            ...
            if (!this.mCurrentEvent.isTainted()) {
                ...
                if (this.mRecentEvents != null) {
                    ...
                    for (int i10 = 0; i10 < 5; i10++) {
                        int index = ((this.mMostRecentEventIndex + 5) - i10) % 5;
                        InputEvent event = this.mRecentEvents[index];
                        if (event == null) {
                            break;
                        }
                        this.mViolationMessage.append("\n  ");
                        appendEvent(this.mViolationMessage, i10 + 1, event, this.mRecentEventsUnhandled[index]);
                    }
                }
