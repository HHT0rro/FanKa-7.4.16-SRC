...
public class RemoteAnimationDefinition implements Parcelable {
    ...
    public void setCallingPidUid(int pid, int uid) {
        for (int i10 = this.mTransitionAnimationMap.size() - 1; i10 >= 0; i10--) {
            this.mTransitionAnimationMap.valueAt(i10).adapter.setCallingPidUid(pid, uid);
        }
    }

    public void linkToDeath(IBinder.DeathRecipient deathRecipient) {
        for (int i10 = 0; i10 < this.mTransitionAnimationMap.size(); i10++) {
            try {
                this.mTransitionAnimationMap.valueAt(i10).adapter.getRunner().asBinder().linkToDeath(deathRecipient, 0);
    ...
    public void writeToParcel(Parcel dest, int flags) {
        ...
        dest.writeInt(size);
        for (int i10 = 0; i10 < size; i10++) {
            dest.writeInt(this.mTransitionAnimationMap.keyAt(i10));
            dest.writeTypedObject(this.mTransitionAnimationMap.valueAt(i10), flags);
        }
    }
