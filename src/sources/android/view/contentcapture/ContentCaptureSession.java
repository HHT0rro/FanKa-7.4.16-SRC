 ...
 */
    ...
    public abstract void flush(int i10);
    ...
    public abstract void internalNotifyViewTreeEvent(boolean z10);
    ...
    public final ContentCaptureSession createContentCaptureSession(ContentCaptureContext context) {
        ...
        if (ContentCaptureHelper.sDebug) {
            Log.d(TAG, "createContentCaptureSession(" + ((Object) context) + ": parent=" + this.mId + ", child=" + child.mId);
        }
    ...
    public final void destroy() {
        synchronized (this.mLock) {
            ...
            if (arrayList != null) {
                ...
                for (int i10 = 0; i10 < numberChildren; i10++) {
                    ContentCaptureSession child = this.mChildren.get(i10);
                    try {
                        child.destroy();
                    } catch (Exception e2) {
                        Log.w(TAG, "exception destroying child session #" + i10 + ": " + ((Object) e2));
                    }
                }
            }
            onDestroy();
    ...
    public final void notifyViewAppeared(ViewStructure node) {
        ...
        if (isContentCaptureEnabled()) {
            if (!(node instanceof ViewNode.ViewStructureImpl)) {
                throw new IllegalArgumentException("Invalid node class: " + ((Object) node.getClass()));
            }
    ...
    public final void notifyViewDisappeared(AutofillId id2) {
        ...
        if (isContentCaptureEnabled()) {
            internalNotifyViewDisappeared(id2);
        }
    }
    ...
    public final void notifyViewsAppeared(List<ViewStructure> appearedNodes) {
        ...
        if (isContentCaptureEnabled()) {
            for (int i10 = 0; i10 < appearedNodes.size(); i10++) {
                ViewStructure v2 = appearedNodes.get(i10);
                if (!(v2 instanceof ViewNode.ViewStructureImpl)) {
                    throw new IllegalArgumentException("Invalid class: " + ((Object) v2.getClass()));
                }
            }
            internalNotifyViewTreeEvent(true);
            for (int i11 = 0; i11 < appearedNodes.size(); i11++) {
                internalNotifyViewAppeared((ViewNode.ViewStructureImpl) appearedNodes.get(i11));
            }
    ...
    public final void notifyViewsDisappeared(AutofillId hostId, long[] virtualIds) {
        Preconditions.checkArgument(hostId.isNonVirtual(), "hostId cannot be virtual: %s", new Object[]{hostId});
        ...
        if (isContentCaptureEnabled()) {
            ...
            for (long id2 : virtualIds) {
                internalNotifyViewDisappeared(new AutofillId(hostId, id2, this.mId));
            }
            if (CompatChanges.isChangeEnabled(NOTIFY_NODES_DISAPPEAR_NOW_SENDS_TREE_EVENTS)) {
                internalNotifyViewTreeEvent(false);
            }
    ...
    public final void notifyViewTextChanged(AutofillId id2, CharSequence text) {
        Objects.requireNonNull(id2);
        if (isContentCaptureEnabled()) {
            internalNotifyViewTextChanged(id2, text);
        }
    }
    ...
    public static String getStateAsString(int state) {
        return state + " (" + (state == 0 ? GrsBaseInfo.CountryCodeSource.UNKNOWN : DebugUtils.flagsToString(ContentCaptureSession.class, "STATE_", state)) + ")";
    }
