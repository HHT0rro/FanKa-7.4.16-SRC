...
public final class PointerIcon implements Parcelable {
    ...
    public static final Parcelable.Creator<PointerIcon> CREATOR = new Parcelable.Creator<PointerIcon>() { // from class: android.view.PointerIcon.1
        ...
        public PointerIcon createFromParcel(Parcel in) {
            ...
            Bitmap bitmap = (Bitmap) Bitmap.CREATOR.createFromParcel(in);
    ...
    public static PointerIcon getSystemIcon(Context context, int type) {
        ...
        int defStyle = sUseLargeIcons ? 16974631 : 16974638;
        TypedArray a10 = context.obtainStyledAttributes(null, R.styleable.Pointer, 0, defStyle);
        int resourceId = a10.getResourceId(typeIndex, -1);
        a10.recycle();
        if (resourceId == -1) {
    ...
    public boolean equals(Object other) {
        ...
        if (this.mType != otherIcon.mType || (i10 = this.mSystemIconResourceId) != otherIcon.mSystemIconResourceId) {
            return false;
        }
        if (i10 != 0 || (this.mBitmap == otherIcon.mBitmap && this.mHotSpotX == otherIcon.mHotSpotX && this.mHotSpotY == otherIcon.mHotSpotY)) {
            return true;
        }
        return false;
    }
    ...
    private void loadResource(Context context, Resources resources, int resourceId) {
        ...
        try {
            try {
                ...
                TypedArray a10 = resources.obtainAttributes(parser, R.styleable.PointerIcon);
                int bitmapRes = a10.getResourceId(0, 0);
                float hotSpotX = a10.getDimension(1, 0.0f);
                float hotSpotY = a10.getDimension(2, 0.0f);
                a10.recycle();
                if (bitmapRes == 0) {
                ...
                if (drawable instanceof AnimationDrawable) {
                    ...
                    if (frames != 1) {
                        ...
                        for (int i10 = 1; i10 < frames; i10++) {
                            Drawable drawableFrame = animationDrawable.getFrame(i10);
                            ...
                            if (drawableFrame.getIntrinsicWidth() != width || drawableFrame.getIntrinsicHeight() != height) {
                                throw new IllegalArgumentException("The bitmap size of " + i10 + "-th frame is different. All frames should have the exact same size and share the same hotspot.");
                            }
                            BitmapDrawable bitmapDrawableFrame = (BitmapDrawable) drawableFrame;
                            this.mBitmapFrames[i10 - 1] = getBitmapFromDrawable(bitmapDrawableFrame);
                        }
                        drawable2 = drawable3;
                    } else {
    ...
    public String toString() {
        return "PointerIcon{type=" + typeToString(this.mType) + ", hotspotX=" + this.mHotSpotX + ", hotspotY=" + this.mHotSpotY + ", systemIconResourceId=" + this.mSystemIconResourceId + i.f4738d;
    }
