...
public class FocusFinder {
    ...
    public static /* synthetic */ View lambda$new$0(View r10, View v2) {
        if (isValidId(v2.getNextFocusForwardId())) {
            return v2.findUserSetNextFocus(r10, 2);
        }
        return null;
    }

    /* JADX INFO: Access modifiers changed from: package-private */
    public static /* synthetic */ View lambda$new$1(View r10, View v2) {
        if (isValidId(v2.getNextClusterForwardId())) {
            return v2.findUserSetNextKeyboardNavigationCluster(r10, 2);
        }
        return null;
    }
    ...
    private View findNextUserSpecifiedFocus(ViewGroup root, View focused, int direction) {
        ...
        while (userSetNextFocus != null) {
            ...
            boolean z10 = !cycleStep;
            cycleStep = z10;
            if (z10 && (cycleCheck = cycleCheck.findUserSetNextFocus(root, direction)) == userSetNextFocus) {
                return null;
            }
        }
        return null;
    ...
    View findNextFocusInAbsoluteDirection(ArrayList<View> focusables, ViewGroup root, View focused, Rect focusedRect, int direction) {
        ...
        for (int i10 = 0; i10 < numFocusables; i10++) {
            View focusable = focusables.get(i10);
    ...
    public View findNearestTouchable(ViewGroup root, int x10, int y10, int direction, int[] deltas) {
        ...
        int i10 = 0;
        while (i10 < numTouchables) {
            View touchable = touchables.get(i10);
            touchable.getDrawingRect(touchableBounds);
            root.offsetRectBetweenParentAndChild(touchable, touchableBounds, true, true);
            if (focusFinder.isTouchCandidate(x10, y10, touchableBounds, direction)) {
                int distance = Integer.MAX_VALUE;
                switch (direction) {
                    case 17:
                        distance = (x10 - touchableBounds.right) + 1;
                        break;
                    case 33:
                        distance = (y10 - touchableBounds.bottom) + 1;
                        break;
                    case 66:
    ...
    private boolean isTouchCandidate(int x10, int y10, Rect destRect, int direction) {
        switch (direction) {
            case 17:
                return destRect.left <= x10 && destRect.top <= y10 && y10 <= destRect.bottom;
            case 33:
                return destRect.top <= y10 && destRect.left <= x10 && x10 <= destRect.right;
            case 66:
                return destRect.left >= x10 && destRect.top <= y10 && y10 <= destRect.bottom;
            case 130:
                return destRect.top >= y10 && destRect.left <= x10 && x10 <= destRect.right;
    ...
    private static final boolean isValidId(int id2) {
 ...
 */
    ...
    public static final class FocusSorter {
        ...
        public void sort(View[] views, int start, int end, ViewGroup root, boolean isRtl) {
            ...
            for (int i10 = this.mRectPool.size(); i10 < count; i10++) {
                this.mRectPool.add(new Rect());
            }
            for (int i11 = start; i11 < end; i11++) {
                ArrayList<Rect> arrayList = this.mRectPool;
                int i12 = this.mLastPoolRect;
                this.mLastPoolRect = i12 + 1;
                Rect next = arrayList.get(i12);
                views[i11].getDrawingRect(next);
                root.offsetDescendantRectToMyCoords(views[i11], next);
                this.mRectByView.put(views[i11], next);
            }
 ...
 */
        ...
        public void setFocusables(List<View> focusables, View root) {
            this.mRoot = root;
            for (int i10 = 0; i10 < focusables.size(); i10++) {
                this.mOriginalOrdinal.put(focusables.get(i10), Integer.valueOf(i10));
            }
            int i11 = focusables.size();
            for (int i12 = i11 - 1; i12 >= 0; i12--) {
                View view = focusables.get(i12);
