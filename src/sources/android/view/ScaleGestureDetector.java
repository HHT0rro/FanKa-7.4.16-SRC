 ...
 */
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        for (int i11 = 0; i11 < count; i11++) {
            if (skipIndex != i11) {
                devSumX += Math.abs(event.getX(i11) - focusX);
                devSumY += Math.abs(event.getY(i11) - focusY);
            }
        }
        ...
        if (scales && this.mGestureDetector == null) {
            GestureDetector.SimpleOnGestureListener gestureListener = new GestureDetector.SimpleOnGestureListener() { // from class: android.view.ScaleGestureDetector.1
                ...
                public boolean onDoubleTap(MotionEvent e2) {
                    ScaleGestureDetector.this.mAnchoredScaleStartX = e2.getX();
                    ScaleGestureDetector.this.mAnchoredScaleStartY = e2.getY();
    ...
    public float getScaleFactor() {
        if (inAnchoredScaleMode()) {
            boolean z10 = this.mEventBeforeOrAboveStartingGestureEvent;
            boolean scaleUp = (z10 && this.mCurrSpan < this.mPrevSpan) || (!z10 && this.mCurrSpan > this.mPrevSpan);
