 ...
 */
    ...
    public int getOffsetForPosition(float x10, float y10) {
        ...
        getLineInfo(y10, lineRange, lineBounds);
        ...
        while (graphemeStart2 != -1 && graphemeStart2 < lineLimit) {
            if (graphemeStart2 >= lineStart) {
                ...
                float distance = Math.abs(cursorPosition - x10);
    ...
    private float getCursorHorizontalPosition(int index, int lineStart, int lineEnd, float lineLeft, float lineRight) {
        int targetIndex;
        boolean isStart;
        Preconditions.checkArgumentInRange(index, lineStart, lineEnd, Attributes.Style.INDEX);
        ...
        float[] fArr = this.mCharacterBounds;
        int i10 = offset * 4;
        return isRtl != isStart ? fArr[i10] : fArr[i10 + 2];
    }
    ...
    private void getLineInfo(float y10, int[] characterRange, RectF bounds) {
        characterRange[0] = -1;
        characterRange[1] = -1;
        ...
        while (currentLineStart != -1 && currentLineStart < this.mEnd) {
            ...
            float distance = verticalDistance(currentLineBounds, y10);
            if (distance == 0.0f) {
    ...
    private static float verticalDistance(RectF rectF, float y10) {
        if (rectF.top <= y10 && y10 < rectF.bottom) {
            return 0.0f;
        }
        if (y10 < rectF.top) {
            return rectF.top - y10;
        }
        return y10 - rectF.bottom;
    }
    ...
    private TextBoundsInfo(Parcel source) {
        ...
        for (int i10 = 0; i10 < length; i10++) {
            this.mInternalCharacterFlags[i10] = encodedFlags[i10] & 66584591;
        }
    }

    private TextBoundsInfo(Builder builder) {
        int i10 = builder.mStart;
        this.mStart = i10;
        int i11 = builder.mEnd;
        this.mEnd = i11;
 ...
 */
    ...
    public static final class Builder {
        ...
        public TextBoundsInfo build() {
            ...
            if (!TextBoundsInfo.isLineDirectionFlagConsistent(iArr, segmentFinder, i11, i10)) {
    ...
    private static void encodeSegmentFinder(int[] flags, int segmentStartFlag, int segmentEndFlag, int start, int end, SegmentFinder segmentFinder) {
        ...
        while (segmentEnd != -1 && segmentEnd <= end) {
            if (segmentStart >= start) {
                int i10 = segmentStart - start;
                flags[i10] = flags[i10] | segmentStartFlag;
                int i11 = segmentEnd - start;
                flags[i11] = flags[i11] | segmentEndFlag;
            }
