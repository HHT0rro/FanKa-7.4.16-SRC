...
public class CutoutSpecification {
    ...
    private void scaleBounds(Rect r10, float ratio) {
 ...
 */
    ...
    public static class Parser {
        ...
        private void computeBoundsRectAndAddToRegion(Path p10, Region inoutRegion, Rect inoutRect) {
            this.mTmpRectF.setEmpty();
            p10.computeBounds(this.mTmpRectF, false);
            this.mTmpRectF.round(inoutRect);
            inoutRegion.op(inoutRect, Region.Op.UNION);
        }

        private void resetStatus(StringBuilder sb2) {
            sb2.setLength(0);
            this.mPositionFromBottom = false;
            this.mPositionFromLeft = false;
            this.mPositionFromRight = false;
        ...
        private int computeSafeInsets(int gravity, Rect rect) {
            ...
            if (gravity == 5 && rect.left > 0) {
                int i10 = rect.left;
                int i11 = this.mPhysicalDisplayWidth;
                if (i10 < i11) {
            ...
            int i12 = rect.top;
            int i13 = this.mPhysicalDisplayHeight;
            if (i12 < i13) {
                return i13 - rect.top;
            }
            return 0;
        ...
        private Rect onSetEdgeCutout(boolean isStart, boolean isShortEdge, Rect rect) {
            ...
            if (isShortEdge) {
                ...
                boolean z10 = this.mIsTouchShortEdgeStart;
                if (z10 && this.mIsTouchShortEdgeEnd) {
                    gravity = CutoutSpecification.decideWhichEdge(this.mIsShortEdgeOnTop, false, isStart);
                } else {
                    gravity = (z10 || this.mIsTouchShortEdgeEnd) ? CutoutSpecification.decideWhichEdge(this.mIsShortEdgeOnTop, true, this.mIsCloserToStartSide) : CutoutSpecification.decideWhichEdge(this.mIsShortEdgeOnTop, isShortEdge, isStart);
                }
            }
        ...
        private void parseSpecWithoutDp(String specWithoutDp) {
            ...
            while (true) {
                ...
                if (sb2 == null) {
                    sb2 = new StringBuilder(specWithoutDp.length());
                }
                sb2.append((CharSequence) specWithoutDp, lastIndex, currentIndex2);
                if (specWithoutDp.startsWith(CutoutSpecification.LEFT_MARKER, currentIndex2)) {
                    ...
                    parseSvgPathSpec(region, sb2.toString());
            ...
            if (sb2 == null) {
                ...
                sb2.append((CharSequence) specWithoutDp, lastIndex, specWithoutDp.length());
                parseSvgPathSpec(region, sb2.toString());
            }
            region.recycle();
        }
