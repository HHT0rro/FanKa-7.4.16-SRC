...
public final class AutofillClientController implements AutofillManager.AutofillClient, Dumpable {
    ...
    public void onActivityPerformResume(boolean followedByPause) {
        if (this.mAutoFillResetNeeded) {
            ...
            if (followedByPause && DEBUG) {
                Slog.v(TAG, "autofill will ignore first pause when relaunching " + ((Object) this));
            }
        }
    }
    ...
    public void onActivityPaused() {
        if (this.mAutoFillResetNeeded) {
            if (!this.mAutoFillIgnoreFirstResumePause) {
                if (DEBUG) {
                    Log.v(TAG, "autofill notifyViewExited " + ((Object) this));
                }
            ...
            if (DEBUG) {
                Log.v(TAG, "autofill got first pause " + ((Object) this));
            }
    ...
    public int getNextAutofillId() {
        ...
        int i10 = this.mLastAutofillId + 1;
        this.mLastAutofillId = i10;
        return i10;
    }
    ...
    public boolean[] autofillClientGetViewVisibility(AutofillId[] autofillIds) {
        ...
        for (int i10 = 0; i10 < autofillIdCount; i10++) {
            AutofillId autofillId = autofillIds[i10];
            if (autofillId == null) {
                visible[i10] = false;
            } else {
                ...
                if (view != null) {
                    if (!autofillId.isVirtualInt()) {
                        visible[i10] = view.isVisibleToUser();
                    } else {
                        visible[i10] = view.isVisibleToUserForAutofill(autofillId.getVirtualChildIntId());
                    }
                }
            }
        }
    ...
    public void autofillClientAuthenticate(int authenticationId, IntentSender intent, Intent fillInIntent, boolean authenticateInline) {
        try {
            ...
            Log.e(TAG, "authenticate() failed for intent:" + ((Object) intent), e2);
        }
    }
}
