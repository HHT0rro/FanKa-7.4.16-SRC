...
public class TableLayout extends LinearLayout {
    ...
    public TableLayout(Context context, AttributeSet attrs) {
        super(context, attrs);
        TypedArray a10 = context.obtainStyledAttributes(attrs, R.styleable.TableLayout);
        String stretchedColumns = a10.getString(0);
        ...
        String shrinkedColumns = a10.getString(1);
        ...
        String collapsedColumns = a10.getString(2);
    ...
    public void trackCollapsedColumns(View child) {
        if (child instanceof TableRow) {
            ...
            for (int i10 = 0; i10 < count; i10++) {
                int columnIndex = collapsedColumns.keyAt(i10);
                boolean isCollapsed = collapsedColumns.valueAt(i10);
                if (isCollapsed) {
        ...
        for (int i10 = 0; i10 < count; i10++) {
            ...
            if (child.getVisibility() == 8) {
                ...
                if (firstRow2) {
                    ...
                    System.arraycopy((Object) widths, 0, (Object) this.mMaxWidths, 0, newLength);
                    firstRow2 = false;
                } else {
                    ...
                    if (difference <= 0) {
                        ...
                        firstRow = firstRow2;
                        System.arraycopy((Object) oldMaxWidths, 0, (Object) iArr2, 0, oldMaxWidths.length);
                        System.arraycopy((Object) widths, oldMaxWidths.length, (Object) this.mMaxWidths, oldMaxWidths.length, difference);
                    }
                    int[] maxWidths = this.mMaxWidths;
                    int length2 = Math.min(length, newLength);
                    for (int j10 = 0; j10 < length2; j10++) {
                        maxWidths[j10] = Math.max(maxWidths[j10], widths[j10]);
                    }
                }
            } else {
                firstRow = firstRow2;
    ...
    private void mutateColumnsWidth(SparseBooleanArray columns, boolean allColumns, int size, int totalWidth) {
        ...
        for (int i10 = 0; i10 < nbChildren; i10++) {
            View child = getChildAt(i10);
            if (child instanceof TableRow) {
                child.forceLayout();
            }
        ...
        if (!allColumns) {
            for (int i11 = 0; i11 < count; i11++) {
                int column = columns.keyAt(i11);
                if (columns.valueAt(i11)) {
                    if (column < length) {
            ...
            if (skipped > 0 && skipped < count) {
                ...
                for (int i12 = 0; i12 < count; i12++) {
                    int column2 = columns.keyAt(i12);
                    if (columns.valueAt(i12) && column2 < length) {
        ...
        for (int i13 = 0; i13 < count; i13++) {
            maxWidths[i13] = maxWidths[i13] + extraSpace;
        }
    }
