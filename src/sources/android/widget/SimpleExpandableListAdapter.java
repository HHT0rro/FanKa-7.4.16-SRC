...
public class SimpleExpandableListAdapter extends BaseExpandableListAdapter {
    ...
    public View getChildView(int groupPosition, int childPosition, boolean isLastChild, View convertView, ViewGroup parent) {
        View v2;
        if (convertView == null) {
            v2 = newChildView(isLastChild, parent);
        } else {
            v2 = convertView;
        }
        bindView(v2, this.mChildData.get(groupPosition).get(childPosition), this.mChildFrom, this.mChildTo);
        return v2;
    }
    ...
    private void bindView(View view, Map<String, ?> data, String[] from, int[] to) {
        ...
        for (int i10 = 0; i10 < len; i10++) {
            TextView v2 = (TextView) view.findViewById(to[i10]);
            if (v2 != null) {
                v2.setText((String) data.get(from[i10]));
            }
        }
    }
    ...
    public View getGroupView(int groupPosition, boolean isExpanded, View convertView, ViewGroup parent) {
        View v2;
        if (convertView == null) {
            v2 = newGroupView(isExpanded, parent);
        } else {
            v2 = convertView;
        }
        bindView(v2, this.mGroupData.get(groupPosition), this.mGroupFrom, this.mGroupTo);
        return v2;
    }
