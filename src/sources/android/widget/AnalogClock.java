 ...
 */
    ...
    public AnalogClock(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        ...
        this.mSecondsHandFps = AppGlobals.getIntCoreSetting(WidgetFlags.KEY_ANALOG_CLOCK_SECONDS_HAND_FPS, context.getResources().getInteger(17694790));
        TypedArray a10 = context.obtainStyledAttributes(attrs, R.styleable.AnalogClock, defStyleAttr, defStyleRes);
        saveAttributeDataForStyleable(context, R.styleable.AnalogClock, attrs, a10, defStyleAttr, defStyleRes);
        Drawable drawable = a10.getDrawable(0);
        this.mDial = drawable;
        if (drawable == null) {
            this.mDial = context.getDrawable(17302131);
        }
        ColorStateList dialTintList = a10.getColorStateList(5);
        if (dialTintList != null) {
            tintInfo4.mTintList = dialTintList;
            tintInfo4.mHasTintList = true;
        }
        BlendMode dialTintMode = Drawable.parseBlendMode(a10.getInt(6, -1), null);
        ...
        Drawable drawable2 = a10.getDrawable(1);
        this.mHourHand = drawable2;
        if (drawable2 == null) {
            this.mHourHand = context.getDrawable(17302132);
        }
        ColorStateList hourHandTintList = a10.getColorStateList(7);
        if (hourHandTintList != null) {
            tintInfo.mTintList = hourHandTintList;
            tintInfo.mHasTintList = true;
        }
        BlendMode hourHandTintMode = Drawable.parseBlendMode(a10.getInt(8, -1), null);
        ...
        Drawable drawable3 = a10.getDrawable(2);
        this.mMinuteHand = drawable3;
        if (drawable3 == null) {
            this.mMinuteHand = context.getDrawable(17302133);
        }
        ColorStateList minuteHandTintList = a10.getColorStateList(9);
        if (minuteHandTintList != null) {
            tintInfo2.mTintList = minuteHandTintList;
            tintInfo2.mHasTintList = true;
        }
        BlendMode minuteHandTintMode = Drawable.parseBlendMode(a10.getInt(10, -1), null);
        ...
        this.mSecondHand = a10.getDrawable(4);
        ColorStateList secondHandTintList = a10.getColorStateList(11);
        if (secondHandTintList != null) {
            tintInfo3.mTintList = secondHandTintList;
            tintInfo3.mHasTintList = true;
        }
        BlendMode secondHandTintMode = Drawable.parseBlendMode(a10.getInt(12, -1), null);
        ...
        this.mTimeZone = toZoneId(a10.getString(3));
        createClock();
        a10.recycle();
    ...
    public void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        float hScale = 1.0f;
        float vScale = 1.0f;
        if (widthMode != 0 && widthSize < (i11 = this.mDialWidth)) {
            hScale = widthSize / i11;
        }
        if (heightMode != 0 && heightSize < (i10 = this.mDialHeight)) {
            vScale = heightSize / i10;
        }
    ...
    public void onDraw(Canvas canvas) {
        ...
        int x10 = availableWidth / 2;
        int y10 = availableHeight / 2;
        Drawable dial = this.mDial;
        int w3 = dial.getIntrinsicWidth();
        int h10 = dial.getIntrinsicHeight();
        boolean scaled = false;
        if (availableWidth < w3 || availableHeight < h10) {
            scaled = true;
            float scale = Math.min(availableWidth / w3, availableHeight / h10);
            canvas.save();
            canvas.scale(scale, scale, x10, y10);
        }
        if (changed) {
            dial.setBounds(x10 - (w3 / 2), y10 - (h10 / 2), (w3 / 2) + x10, (h10 / 2) + y10);
        }
        dial.draw(canvas);
        canvas.save();
        canvas.rotate((this.mHour / 12.0f) * 360.0f, x10, y10);
        Drawable hourHand = this.mHourHand;
        if (changed) {
            int w10 = hourHand.getIntrinsicWidth();
            int h11 = hourHand.getIntrinsicHeight();
            hourHand.setBounds(x10 - (w10 / 2), y10 - (h11 / 2), (w10 / 2) + x10, y10 + (h11 / 2));
        }
        hourHand.draw(canvas);
        canvas.restore();
        canvas.save();
        canvas.rotate((this.mMinutes / 60.0f) * 360.0f, x10, y10);
        Drawable minuteHand = this.mMinuteHand;
        if (changed) {
            int w11 = minuteHand.getIntrinsicWidth();
            int h12 = minuteHand.getIntrinsicHeight();
            minuteHand.setBounds(x10 - (w11 / 2), y10 - (h12 / 2), x10 + (w11 / 2), y10 + (h12 / 2));
        }
        ...
        if (secondHand != null && this.mSecondsHandFps > 0) {
            canvas.save();
            canvas.rotate((this.mSeconds / 60.0f) * 360.0f, x10, y10);
            if (changed) {
                int w12 = secondHand.getIntrinsicWidth();
                int h13 = secondHand.getIntrinsicHeight();
                secondHand.setBounds(x10 - (w12 / 2), y10 - (h13 / 2), (w12 / 2) + x10, y10 + (h13 / 2));
            }
    ...
    public void onTimeChanged(LocalTime localTime, long nowMillis) {
        ...
        float f10 = this.mMinutes;
        float f11 = hour + (f10 / 60.0f);
        this.mHour = f11;
        this.mChanged = true;
        if (((int) previousHour) != ((int) f11) || ((int) previousMinutes) != ((int) f10)) {
    ...
    private static ZoneId toZoneId(String timeZone) {
        ...
        try {
            ...
            Log.w(LOG_TAG, "Failed to parse time zone from " + timeZone, e2);
            return null;
        }
    }
