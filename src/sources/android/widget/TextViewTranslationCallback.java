...
public class TextViewTranslationCallback implements ViewTranslationCallback {
    ...
    private void clearTranslationTransformation() {
        if (DEBUG) {
            Log.v(TAG, "clearTranslationTransformation: " + ((Object) this.mTranslationTransformation));
        }
    ...
    public boolean onShowTranslation(View view) {
        if (this.mIsShowingTranslation) {
            if (DEBUG) {
                Log.d(TAG, ((Object) view) + " is already showing translated text.");
            }
            return false;
        }
        ...
        runChangeTextWithAnimationIfNeeded(theTextView, new Runnable() { // from class: android.widget.TextViewTranslationCallback$$ExternalSyntheticLambda2
            ...
            public final void run() {
                TextViewTranslationCallback.this.lambda$onShowTranslation$0(textViewRef, translationTransformationMethod2);
            }
        });
    ...
    private CharSequence computePaddedText(CharSequence text, CharSequence translatedText) {
        ...
        StringBuilder sb2 = new StringBuilder(newLength);
        sb2.append(text);
        for (int i10 = text.length(); i10 < newLength; i10++) {
            sb2.append(COMPAT_PAD_CHARACTER);
        }
        return sb2;
    }
