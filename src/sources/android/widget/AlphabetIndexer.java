...
public class AlphabetIndexer extends DataSetObserver implements SectionIndexer {
    ...
    public AlphabetIndexer(Cursor cursor, int sortedColumnIndex, CharSequence alphabet) {
        ...
        int i11 = 0;
        while (true) {
            i10 = this.mAlphabetLength;
            if (i11 >= i10) {
                break;
            }
            this.mAlphabetArray[i11] = Character.toString(this.mAlphabet.charAt(i11));
            i11++;
        }
        this.mAlphaMap = new SparseIntArray(i10);
        if (cursor != null) {
    ...
    public int getSectionForPosition(int position) {
        ...
        for (int i10 = 0; i10 < this.mAlphabetLength; i10++) {
            char letter = this.mAlphabet.charAt(i10);
            String targetLetter = Character.toString(letter);
