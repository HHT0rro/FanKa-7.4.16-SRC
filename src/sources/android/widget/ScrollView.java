...
public class ScrollView extends FrameLayout {
    ...
    private IOplusScrollViewExt mOplusScrollViewExt;
    ...
    private final IScrollViewWrapper mWrapper;
 ...
 */
    ...
    public ScrollView(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        ...
        this.mOplusScrollViewExt = (IOplusScrollViewExt) ExtLoader.type(IOplusScrollViewExt.class).base(this).create();
        this.mWrapper = new ScrollViewWrapper();
        this.mEdgeGlowTop = new EdgeEffect(context, attrs);
        this.mEdgeGlowBottom = new EdgeEffect(context, attrs);
        initScrollView();
        TypedArray a10 = context.obtainStyledAttributes(attrs, R.styleable.ScrollView, defStyleAttr, defStyleRes);
        saveAttributeDataForStyleable(context, R.styleable.ScrollView, attrs, a10, defStyleAttr, defStyleRes);
    ...
    private boolean inChild(int x10, int y10) {
        ...
        View child = getChildAt(0);
        return y10 >= child.getTop() - scrollY && y10 < child.getBottom() - scrollY && x10 >= child.getLeft() && x10 < child.getRight();
    }
            ...
            case 2:
                ...
                if (activePointerId != -1) {
                    ...
                    if (pointerIndex == -1) {
                        ...
                        int y11 = (int) ev.getY(pointerIndex);
                        int yDiff = Math.abs(y11 - this.mLastMotionY);
        ...
        switch (actionMasked) {
            ...
            case 2:
                ...
                if (activePointerIndex == -1) {
                    ...
                    int y10 = (int) ev.getY(activePointerIndex);
                    int i10 = this.mLastMotionY;
                    int deltaY2 = i10 - y10;
                    this.mOplusScrollViewExt.markOnTouchEventMove(y10, deltaY2, i10);
    ...
    protected void onOverScrolled(int scrollX, int scrollY, boolean clampedX, boolean clampedY) {
        if (!this.mScroller.isFinished()) {
            ...
            this.mOplusScrollViewExt.markOnOverScrolled(this.mScrollY - scrollY);
    ...
    public void computeScroll() {
        if (this.mScroller.computeScrollOffset()) {
            ...
            int deltaY = consumeFlingInStretch(y10 - oldY);
            if (oldX != x10 || deltaY != 0) {
                int range = getScrollRange();
                int overscrollMode = getOverScrollMode();
                boolean z10 = true;
                if (overscrollMode != 0 && (overscrollMode != 1 || range <= 0)) {
                ...
                boolean canOverscroll = z10;
                overScrollBy(x10 - oldX, deltaY, oldX, oldY, 0, range, 0, this.mOverflingDistance, false);
                onScrollChanged(this.mScrollX, this.mScrollY, oldX, oldY);
                if (canOverscroll && deltaY != 0) {
                    if (y10 < 0 && oldY >= 0) {
                        this.mEdgeGlowTop.onAbsorb((int) this.mScroller.getCurrVelocity());
                    } else if (y10 > range && oldY <= range) {
    ...
    public void onLayout(boolean changed, int l10, int t2, int r10, int b4) {
        ...
        if (!isLaidOut()) {
            ...
            int scrollRange = Math.max(0, childHeight - (((b4 - t2) - this.mPaddingBottom) - this.mPaddingTop));
            if (this.mScrollY > scrollRange) {
    ...
    private void flingWithNestedDispatch(int velocityY) {
        ...
        if (!dispatchNestedPreFling(0.0f, velocityY)) {
            ...
            if (this.mOplusScrollViewExt.canFling(canFling)) {
                fling(velocityY);
                return;
            }
            if (!consumed) {
    ...
    public void draw(Canvas canvas) {
        ...
        float translateY2;
        super.draw(canvas);
        if (this.mOplusScrollViewExt.shouldDisplayEdgeEffects(shouldDisplayEdgeEffects())) {
            int scrollY = this.mScrollY;
 ...
 */
    ...
    public static class SavedState extends View.BaseSavedState {
        ...
        public String toString() {
            return "ScrollView.SavedState{" + Integer.toHexString(System.identityHashCode(this)) + " scrollPosition=" + this.scrollPosition + i.f4738d;
        }
    }

    public IScrollViewWrapper getWrapper() {
 ...
 */
    /* loaded from: C:\Users\35037\Desktop\fankahook\2\class11.dex.bak */
    private class ScrollViewWrapper implements IScrollViewWrapper {
        private ScrollViewWrapper() {
        }

        @Override // android.widget.IScrollViewWrapper
        public IOplusScrollViewExt getExtImpl() {
            return ScrollView.this.mOplusScrollViewExt;
        }

        @Override // android.widget.IScrollViewWrapper
        public OverScroller getOverScroller() {
            return ScrollView.this.mScroller;
        }
    }
}
