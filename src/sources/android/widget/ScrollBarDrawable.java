...
public class ScrollBarDrawable extends Drawable implements Drawable.Callback {
    ...
    public IScrollBarDrawableExt mScrollBarDrawableExt = (IScrollBarDrawableExt) ExtLoader.type(IScrollBarDrawableExt.class).base(this).create();
    ...
    public void draw(Canvas canvas) {
        ...
        Rect r10 = getBounds();
        if (canvas.quickReject(r10.left, r10.top, r10.right, r10.bottom)) {
            return;
        }
        this.mScrollBarDrawableExt.getDrawRect(r10);
        if (drawTrack) {
            drawTrack(canvas, r10, vertical);
        }
        if (drawThumb) {
            int scrollBarLength = vertical ? r10.height() : r10.width();
            int thickness = vertical ? r10.width() : r10.height();
            int thumbLength = this.mScrollBarDrawableExt.getThumbLength(scrollBarLength, thickness, extent, range);
            int thumbOffset = ScrollBarUtils.getThumbOffset(scrollBarLength, thumbLength, extent, range, this.mOffset);
            drawThumb(canvas, r10, thumbOffset, thumbLength, vertical);
        }
    }
    ...
    private void propagateCurrentState(Drawable d10) {
        if (d10 != null) {
            if (this.mMutated) {
                d10.mutate();
            }
            d10.setState(getState());
            d10.setCallback(this);
            if (this.mHasSetAlpha) {
                d10.setAlpha(this.mAlpha);
            }
            if (this.mHasSetColorFilter) {
                d10.setColorFilter(this.mColorFilter);
            }
        }
    }
