 ...
 */
    ...
    static {
        ...
        for (int i10 = 0; i10 < 12; i10++) {
            COS_30[i10] = (float) Math.cos(angle);
            SIN_30[i10] = (float) Math.sin(angle);
    ...
    /* JADX WARN: Type inference failed for: r2v7, types: [android.widget.RadialTimePickerView$RadialPickerTouchHelper, android.view.View$AccessibilityDelegate] */
    public RadialTimePickerView(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        ...
        this.mAmOrPm = 0;
        ?? radialPickerTouchHelper = new RadialPickerTouchHelper();
    ...
    public void applyAttributes(AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        ...
        TypedArray a10 = getContext().obtainStyledAttributes(attrs, R.styleable.TimePicker, defStyleAttr, defStyleRes);
        saveAttributeDataForStyleable(context, R.styleable.TimePicker, attrs, a10, defStyleAttr, defStyleRes);
        ColorStateList numbersTextColor = a10.getColorStateList(3);
        ColorStateList numbersInnerTextColor = a10.getColorStateList(9);
        this.mTextColor[0] = numbersTextColor == null ? ColorStateList.valueOf(MISSING_COLOR) : numbersTextColor;
        ...
        ColorStateList selectorColors = a10.getColorStateList(5);
        ...
        this.mPaintBackground.setColor(a10.getColor(4, context.getColor(R.color.timepicker_default_numbers_background_color_material)));
        a10.recycle();
    }
    ...
    private void initHoursAndMinutesText() {
        for (int i10 = 0; i10 < 12; i10++) {
            ...
            this.mInnerHours24Texts[i10] = String.format("%02d", Integer.valueOf(HOURS_NUMBERS_24[i10]));
            this.mOuterHours24Texts[i10] = String.format("%d", Integer.valueOf(iArr[i10]));
    ...
    public void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        int i10 = iArr[2];
        int i11 = this.mSelectorRadius;
        this.mMinDistForInnerNumber = (min - i10) - i11;
        int i12 = iArr[0];
        this.mMaxDistForOuterNumber = (min - i12) + i11;
        this.mHalfwayDist = min - ((i12 + i10) / 2);
    ...
    private void drawSelector(Canvas canvas, Path selectorPath) {
        ...
        int i11 = this.mCenterDotRadius;
        int centerX = i10 + ((int) (i11 * sin));
        int centerY = this.mYCenter - ((int) (i11 * cos));
    ...
    private static void calculatePositions(Paint paint, float radius, float xCenter, float yCenter, float textSize, float[] x10, float[] y10) {
        paint.setTextSize(textSize);
        float yCenter2 = yCenter - ((paint.descent() + paint.ascent()) / 2.0f);
        for (int i10 = 0; i10 < 12; i10++) {
            x10[i10] = xCenter - (COS_30[i10] * radius);
            y10[i10] = yCenter2 - (SIN_30[i10] * radius);
        }
    }
    ...
    private void drawTextElements(Canvas canvas, float textSize, Typeface typeface, ColorStateList textColor, String[] texts, float[] textX, float[] textY, Paint paint, int alpha, boolean showActivated, int activatedDegrees, boolean activatedOnly) {
        ...
        for (int i11 = 12; i10 < i11; i11 = 12) {
            boolean activated = activatedFloor == i10 || activatedCeil == i10;
            if (!activatedOnly || activated) {
                ...
                canvas.drawText(texts[i10], textX[i10], textY[i10], paint);
            }
            i10++;
        }
    }
    ...
    /* JADX INFO: Access modifiers changed from: private */
    public int getDegreesFromXY(float f10, float f11, boolean z10) {
        int i10;
        int i11;
        if (this.mIs24HourMode && this.mShowHours) {
            i11 = this.mMinDistForInnerNumber;
            i10 = this.mMaxDistForOuterNumber;
        } else {
            int i12 = this.mCircleRadius - this.mTextInset[!this.mShowHours ? 1 : 0];
            int i13 = this.mSelectorRadius;
            int i14 = i12 - i13;
            i10 = i13 + i12;
            i11 = i14;
        }
        double d10 = f10 - this.mXCenter;
        double d11 = f11 - this.mYCenter;
    ...
    /* JADX INFO: Access modifiers changed from: private */
    public boolean getInnerCircleFromXY(float x10, float y10) {
        if (!this.mIs24HourMode || !this.mShowHours) {
            return false;
        }
        double dX = x10 - this.mXCenter;
        double dY = y10 - this.mYCenter;
    ...
    private boolean handleTouchInput(float x10, float y10, boolean forceSelection, boolean autoAdvance) {
        int snapDegrees;
        int type;
        boolean valueChanged;
        boolean isOnInnerCircle = getInnerCircleFromXY(x10, y10);
        int degrees = getDegreesFromXY(x10, y10, false);
        if (degrees == -1) {
            return false;
        }
 ...
 */
    ...
    public class RadialPickerTouchHelper extends ExploreByTouchHelper {
        ...
        protected int getVirtualViewAt(float x10, float y10) {
            int minute;
            int degrees = RadialTimePickerView.this.getDegreesFromXY(x10, y10, true);
            if (degrees != -1) {
                int snapDegrees = RadialTimePickerView.snapOnly30s(degrees, 0) % 360;
                if (RadialTimePickerView.this.mShowHours) {
                    boolean isOnInnerCircle = RadialTimePickerView.this.getInnerCircleFromXY(x10, y10);
        ...
        protected void getVisibleVirtualViews(IntArray intArray) {
            if (RadialTimePickerView.this.mShowHours) {
                int i10 = !RadialTimePickerView.this.mIs24HourMode ? 1 : 0;
                int i11 = RadialTimePickerView.this.mIs24HourMode ? 23 : 12;
                for (int i12 = i10; i12 <= i11; i12++) {
                    intArray.add(makeId(1, i12));
                }
                return;
            }
            int currentMinute = RadialTimePickerView.this.getCurrentMinute();
            for (int i13 = 0; i13 < 60; i13 += 5) {
                intArray.add(makeId(2, i13));
                if (currentMinute > i13 && currentMinute < i13 + 5) {
