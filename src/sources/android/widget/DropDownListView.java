...
public class DropDownListView extends ListView {
        ...
        switch (actionMasked) {
            ...
            case 2:
                ...
                if (activeIndex < 0) {
                    ...
                    int x10 = (int) event.getX(activeIndex);
                    int y10 = (int) event.getY(activeIndex);
                    ...
                    if (position == -1) {
                        ...
                        setPressedItem(child, position, x10, y10);
                        handledEvent = true;
                        if (actionMasked == 1) {
                            long id2 = getItemIdAtPosition(position);
                            performItemClick(child, position, id2);
                            break;
                        }
                    }
                }
    ...
    private void setPressedItem(View child, int position, float x10, float y10) {
        ...
        float childX = x10 - child.getLeft();
        float childY = y10 - child.getTop();
        child.drawableHotspotChanged(childX, childY);
        if (!child.isPressed()) {
            child.setPressed(true);
        }
        setSelectedPositionInt(position);
        positionSelectorLikeTouch(position, child, x10, y10);
