...
public class CalendarViewLegacyDelegate extends CalendarView.AbstractCalendarViewDelegate {
    ...
    public CalendarViewLegacyDelegate(CalendarView delegator, Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        ...
        this.mShowWeekNumber = a10.getBoolean(1, true);
        this.mFirstDayOfWeek = a10.getInt(0, Calendar.getInstance().getFirstDayOfWeek());
        ...
        if (!this.mMaxDate.before(this.mMinDate)) {
            this.mShownWeekCount = a10.getInt(4, 6);
            this.mSelectedWeekBackgroundColor = a10.getColor(5, 0);
            this.mFocusedMonthDateColor = a10.getColor(6, 0);
            this.mUnfocusedMonthDateColor = a10.getColor(7, 0);
            this.mWeekSeparatorLineColor = a10.getColor(9, 0);
            this.mWeekNumberColor = a10.getColor(8, 0);
            this.mSelectedDateVerticalBar = a10.getDrawable(10);
            this.mDateTextAppearanceResId = a10.getResourceId(12, 16973894);
            updateDateTextSize();
            this.mWeekDayTextAppearanceResId = a10.getResourceId(11, -1);
            a10.recycle();
            ...
            View content = layoutInflater.inflate(17367113, (ViewGroup) null, false);
            this.mDelegator.addView(content);
            this.mListView = (ListView) this.mDelegator.findViewById(16908298);
            this.mDayNamesHeader = (ViewGroup) content.findViewById(16908955);
            this.mMonthName = (TextView) content.findViewById(16909267);
            setUpHeader();
            setUpListView();
            setUpAdapter();
    ...
    public void setSelectedWeekBackgroundColor(int color) {
        if (this.mSelectedWeekBackgroundColor != color) {
            ...
            for (int i10 = 0; i10 < childCount; i10++) {
                WeekView weekView = (WeekView) this.mListView.getChildAt(i10);
    ...
    public boolean getBoundsForDate(long date, Rect outBounds) {
        ...
        for (int i10 = 0; i10 < listViewEntryCount; i10++) {
            WeekView currWeekView = (WeekView) this.mListView.getChildAt(i10);
    ...
    private void invalidateAllWeekViews() {
        int childCount = this.mListView.getChildCount();
        for (int i10 = 0; i10 < childCount; i10++) {
            View view = this.mListView.getChildAt(i10);
            view.invalidate();
        }
    }
    ...
    private void setUpHeader() {
        ...
        this.mDayNamesShort = new String[i10];
        this.mDayNamesLong = new String[i10];
        int i11 = this.mFirstDayOfWeek;
        int count = this.mFirstDayOfWeek + i10;
        while (i11 < count) {
            int calendarDay = i11 > 7 ? i11 - 7 : i11;
            this.mDayNamesShort[i11 - this.mFirstDayOfWeek] = DateUtils.getDayOfWeekString(calendarDay, 50);
            this.mDayNamesLong[i11 - this.mFirstDayOfWeek] = DateUtils.getDayOfWeekString(calendarDay, 10);
            i11++;
        }
        ...
        for (int i12 = 1; i12 < count2; i12++) {
            TextView label2 = (TextView) this.mDayNamesHeader.getChildAt(i12);
            int i13 = this.mWeekDayTextAppearanceResId;
            if (i13 > -1) {
                label2.setTextAppearance(i13);
            }
            if (i12 < this.mDaysPerWeek + 1) {
                label2.setText(this.mDayNamesShort[i12 - 1]);
                label2.setContentDescription(this.mDayNamesLong[i12 - 1]);
    ...
    public void onScroll(AbsListView view, int firstVisibleItem, int visibleItemCount, int totalItemCount) {
        ...
        if (child != null) {
            ...
            if ((!z10 && monthDiff > 0) || (z10 && monthDiff < 0)) {
    ...
    public void setMonthDisplayed(Calendar calendar) {
        ...
        this.mAdapter.setFocusMonth(i10);
    ...
    public int getWeeksSinceMinDate(Calendar date) {
        if (date.before(this.mMinDate)) {
            throw new IllegalArgumentException("fromDate: " + ((Object) this.mMinDate.getTime()) + " does not precede toDate: " + ((Object) date.getTime()));
        }
 ...
 */
    ...
    public class WeekView extends View {
        ...
        public void init(int weekNumber, int selectedWeekDay, int focusedMonth) {
            ...
            this.mDayNumbers = new String[i10];
            this.mFocusDay = new boolean[i10];
            int i11 = 0;
            ...
            while (i11 < this.mNumCells) {
                ...
                this.mFocusDay[i11] = isFocusedDay;
                ...
                if (CalendarViewLegacyDelegate.this.mTempDate.before(CalendarViewLegacyDelegate.this.mMinDate) || CalendarViewLegacyDelegate.this.mTempDate.after(CalendarViewLegacyDelegate.this.mMaxDate)) {
                    this.mDayNumbers[i11] = "";
                } else {
                    this.mDayNumbers[i11] = String.format(Locale.getDefault(), "%d", Integer.valueOf(CalendarViewLegacyDelegate.this.mTempDate.get(5)));
                }
        ...
        public boolean getDayFromLocation(float x10, Calendar outCalendar) {
            int start;
            int end;
            ...
            if (x10 < start || x10 > end) {
                outCalendar.clear();
                return false;
            }
            int dayPosition = (int) (((x10 - start) * CalendarViewLegacyDelegate.this.mDaysPerWeek) / (end - start));
            if (isLayoutRtl) {
        ...
        public boolean getBoundsForDate(Calendar date, Rect outBounds) {
            ...
            for (int i10 = 0; i10 < CalendarViewLegacyDelegate.this.mDaysPerWeek; i10++) {
                if (date.get(1) == currDay.get(1) && date.get(2) == currDay.get(2) && date.get(5) == currDay.get(5)) {
                    int cellSize = this.mWidth / this.mNumCells;
                    if (isLayoutRtl()) {
                        outBounds.left = (CalendarViewLegacyDelegate.this.mShowWeekNumber ? (this.mNumCells - i10) - 2 : (this.mNumCells - i10) - 1) * cellSize;
                    } else {
                        outBounds.left = (CalendarViewLegacyDelegate.this.mShowWeekNumber ? i10 + 1 : i10) * cellSize;
                    }
                    outBounds.top = 0;
        ...
        private void drawWeekNumbersAndDates(Canvas canvas) {
            ...
            int y10 = ((int) ((this.mHeight + textHeight) / 2.0f)) - CalendarViewLegacyDelegate.this.mWeekSeparatorLineWidth;
            ...
            if (isLayoutRtl()) {
                while (i10 < nDays - 1) {
                    this.mMonthNumDrawPaint.setColor(this.mFocusDay[i10] ? CalendarViewLegacyDelegate.this.mFocusedMonthDateColor : CalendarViewLegacyDelegate.this.mUnfocusedMonthDateColor);
                    int x10 = (((i10 * 2) + 1) * this.mWidth) / divisor;
                    canvas.drawText(this.mDayNumbers[(nDays - 1) - i10], x10, y10, this.mMonthNumDrawPaint);
                    i10++;
                }
                if (CalendarViewLegacyDelegate.this.mShowWeekNumber) {
                    this.mDrawPaint.setColor(CalendarViewLegacyDelegate.this.mWeekNumberColor);
                    int i11 = this.mWidth;
                    int x11 = i11 - (i11 / divisor);
                    canvas.drawText(this.mDayNumbers[0], x11, y10, this.mDrawPaint);
                    return;
                }
                return;
            }
            if (CalendarViewLegacyDelegate.this.mShowWeekNumber) {
                this.mDrawPaint.setColor(CalendarViewLegacyDelegate.this.mWeekNumberColor);
                int x12 = this.mWidth / divisor;
                canvas.drawText(this.mDayNumbers[0], x12, y10, this.mDrawPaint);
                i10 = 0 + 1;
            }
            while (i10 < nDays) {
                this.mMonthNumDrawPaint.setColor(this.mFocusDay[i10] ? CalendarViewLegacyDelegate.this.mFocusedMonthDateColor : CalendarViewLegacyDelegate.this.mUnfocusedMonthDateColor);
                int x13 = (((i10 * 2) + 1) * this.mWidth) / divisor;
                canvas.drawText(this.mDayNumbers[i10], x13, y10, this.mMonthNumDrawPaint);
                i10++;
            }
        }
