 ...
 */
    ...
    public void addSpellCheckSpan(Editable editable, int start, int end) {
        ...
        int[] iArr = this.mIds;
        int i10 = this.mSpanSequenceCounter;
        this.mSpanSequenceCounter = i10 + 1;
        iArr[index] = i10;
    }

    public void onSpellCheckSpanRemoved(SpellCheckSpan spellCheckSpan) {
        for (int i10 = 0; i10 < this.mLength; i10++) {
            if (this.mSpellCheckSpans[i10] == spellCheckSpan) {
                this.mIds[i10] = -1;
                return;
            }
        }
    ...
    public void spellCheck(int start, int end, boolean forceCheckWhenEditingWord) {
        ...
        if (isSessionActive) {
            int length = this.mSpellParsers.length;
            for (int i10 = 0; i10 < length; i10++) {
                SpellParser spellParser = this.mSpellParsers[i10];
            ...
            int i11 = length + 1;
            SpellParser[] newSpellParsers = new SpellParser[i11];
    ...
    public void spellCheck(boolean forceCheckWhenEditingWord) {
        ...
        while (true) {
            ...
            boolean z10 = false;
            if (i10 >= textInfosCount2) {
                break;
            }
            SpellCheckSpan spellCheckSpan = this.mSpellCheckSpans[i10];
            if (this.mIds[i10] >= 0 && !spellCheckSpan.isSpellCheckInProgress()) {
                ...
                if (start >= 0 && end > start && (forceCheckWhenEditingWord || isNotEditing)) {
                    ...
                    TextInfo textInfo = new TextInfo(editable, start, end, this.mCookie, this.mIds[i10]);
    ...
    private SpellCheckSpan onGetSuggestionsInternal(SuggestionsInfo suggestionsInfo, int offset, int length) {
        ...
        for (int k10 = 0; k10 < this.mLength; k10++) {
            if (sequenceNumber == this.mIds[k10]) {
                SpellCheckSpan spellCheckSpan = this.mSpellCheckSpans[k10];
    ...
    public void onGetSentenceSuggestions(SentenceSuggestionsInfo[] results) {
        ...
        for (SentenceSuggestionsInfo ssi : results) {
            if (ssi != null) {
                ...
                for (int j10 = 0; j10 < ssi.getSuggestionsCount(); j10++) {
                    SuggestionsInfo suggestionsInfo = ssi.getSuggestionsInfoAt(j10);
                    if (suggestionsInfo != null) {
                        int offset = ssi.getOffsetAt(j10);
                        int length = ssi.getLengthAt(j10);
        ...
        if (runnable == null) {
            this.mSpellRunnable = new Runnable() { // from class: android.widget.SpellChecker.1
                ...
                public void run() {
                    ...
                    for (int i10 = 0; i10 < length; i10++) {
                        SpellParser spellParser = SpellChecker.this.mSpellParsers[i10];
                        if (!spellParser.isFinished()) {
                            spellParser.parse();
                            return;
                        }
        ...
        if (spellCheckSpanStart >= 0 && spellCheckSpanEnd > spellCheckSpanStart) {
            ...
            if (suggestionsCount > 0) {
                ...
                for (int i10 = 0; i10 < suggestionsCount; i10++) {
                    suggestions[i10] = suggestionsInfo.getSuggestionAt(i10);
                }
            } else {
 ...
 */
    ...
    public static class SentenceIteratorWrapper {
        ...
        public int preceding(int offset) {
            ...
            if (offset >= i10 && (result = this.mSentenceIterator.preceding(offset - i10)) != -1) {
        ...
        public boolean isBoundary(int offset) {
            int i10 = this.mStartOffset;
            if (offset < i10 || offset > this.mEndOffset) {
                return false;
            }
            return this.mSentenceIterator.isBoundary(offset - i10);
        }
    }
 ...
 */
    ...
    public class SpellParser {
        ...
        public void parse() {
            ...
            int i10 = 0;
            while (true) {
                if (i10 >= SpellChecker.this.mLength) {
                    break;
                }
                SpellCheckSpan spellCheckSpan = SpellChecker.this.mSpellCheckSpans[i10];
                if (SpellChecker.this.mIds[i10] >= 0 && !spellCheckSpan.isSpellCheckInProgress()) {
    ...
    public Range<Integer> detectSentenceBoundary(CharSequence sequence, int textChangeStart, int textChangeEnd) {
        ...
        int iteratorWindowEnd = findSeparator(sequence, Math.min(textChangeStart + 700, textChangeEnd), Math.min(textChangeStart + DownloadErrorCode.ERROR_NO_SDCARD_PERMISSION, sequence.length()));
    ...
    private static int findSeparator(CharSequence sequence, int start, int end) {
        ...
        for (int i10 = start; i10 != end; i10 += step) {
            if (isSeparator(sequence.charAt(i10))) {
                return i10;
            }
        }
        return end;
