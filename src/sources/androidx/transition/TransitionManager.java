...
public class TransitionManager {
    ...
    public static class MultiListener implements ViewTreeObserver.OnPreDrawListener, View.OnAttachStateChangeListener {
        ...
        public boolean onPreDraw() {
            ...
            if (arrayList2 != null) {
                ...
                while (iterator2.hasNext()) {
                    ((Transition) iterator2.next()).resume(this.mSceneRoot);
                }
            }
        ...
        public void onViewDetachedFromWindow(View view) {
            ...
            if (arrayList != null && arrayList.size() > 0) {
                Iterator<Transition> iterator2 = arrayList.iterator2();
                while (iterator2.hasNext()) {
                    iterator2.next().resume(this.mSceneRoot);
                }
            }
    ...
    private static void changeScene(Scene scene, Transition transition) {
        ...
        Transition mo1754clone = transition.mo1754clone();
        mo1754clone.setSceneRoot(sceneRoot);
        if (currentScene != null && currentScene.isCreatedFromLayoutResource()) {
            mo1754clone.setCanRemoveViews(true);
        }
        sceneChangeSetup(sceneRoot, mo1754clone);
        scene.enter();
        sceneChangeRunTransition(sceneRoot, mo1754clone);
    }
    ...
    private static void sceneChangeSetup(ViewGroup viewGroup, Transition transition) {
        ...
        if (arrayList != null && arrayList.size() > 0) {
            Iterator<Transition> iterator2 = arrayList.iterator2();
            while (iterator2.hasNext()) {
                iterator2.next().pause(viewGroup);
            }
        }
        if (transition != null) {
    ...
    public static void beginDelayedTransition(@NonNull ViewGroup viewGroup, @Nullable Transition transition) {
        ...
        Transition mo1754clone = transition.mo1754clone();
        sceneChangeSetup(viewGroup, mo1754clone);
        Scene.setCurrentScene(viewGroup, null);
        sceneChangeRunTransition(viewGroup, mo1754clone);
    }
