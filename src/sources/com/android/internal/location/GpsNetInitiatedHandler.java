 ...
 */
    ...
    public interface EmergencyCallCallback {
        ...
        void onEmergencyCallStart(int i10);
    }
 ...
 */
    ...
    private synchronized void setNiNotification(GpsNiNotification notif) {
        ...
        if (this.mNiNotificationBuilder == null) {
            this.mNiNotificationBuilder = new Notification.Builder(this.mContext, SystemNotificationChannels.NETWORK_ALERTS).setSmallIcon(17303709).setWhen(0L).setOngoing(true).setAutoCancel(true).setColor(this.mContext.getColor(17170460));
        }
        if (this.mPlaySounds) {
    ...
    static byte[] stringToByteArray(String original, boolean isHex) {
        ...
        if (isHex) {
            for (int i10 = 0; i10 < length; i10++) {
                output[i10] = (byte) Integer.parseInt(original.substring(i10 * 2, (i10 * 2) + 2), 16);
            }
        } else {
            for (int i11 = 0; i11 < length; i11++) {
                output[i11] = (byte) original.charAt(i11);
            }
        }
        return output;
    }
    ...
    static String decodeUCS2String(byte[] input) {
        try {
            String decoded = new String(input, CharEncoding.UTF_16);
            return decoded;
        } catch (UnsupportedEncodingException e2) {
            throw new AssertionError();
        }
    ...
    private static String getNotifTicker(GpsNiNotification notif, Context context) {
        String ticker = String.format(context.getString(17040462), decodeString(notif.requestorId, mIsHexInput, notif.requestorIdEncoding), decodeString(notif.text, mIsHexInput, notif.textEncoding));
        return ticker;
    }

    private static String getNotifTitle(GpsNiNotification notif, Context context) {
        String title = String.format(context.getString(17040463), new Object[0]);
        return title;
    }

    private static String getNotifMessage(GpsNiNotification notif, Context context) {
        String message = String.format(context.getString(17040461), decodeString(notif.requestorId, mIsHexInput, notif.requestorIdEncoding), decodeString(notif.text, mIsHexInput, notif.textEncoding));
        return message;
    }
