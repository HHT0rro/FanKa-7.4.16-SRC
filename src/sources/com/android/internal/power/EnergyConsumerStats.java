 ...
 */
    ...
    public EnergyConsumerStats(Config config, Parcel in) {
        ...
        if (in.readBoolean()) {
            ...
            for (int i10 = 0; i10 < size; i10++) {
                if (in.readBoolean()) {
                    this.mAccumulatedMultiStateChargeMicroCoulomb[i10] = LongMultiStateCounter.CREATOR.createFromParcel(in);
                }
            }
            return;
        }
    ...
    public void setState(int state, long timestampMs) {
        ...
        while (true) {
            LongMultiStateCounter[] longMultiStateCounterArr = this.mAccumulatedMultiStateChargeMicroCoulomb;
            if (i10 < longMultiStateCounterArr.length) {
                LongMultiStateCounter counter = longMultiStateCounterArr[i10];
                if (counter == null && this.mConfig.isSupportedMultiStateBucket(i10)) {
                    counter = new LongMultiStateCounter(this.mConfig.mStateNames.length);
                    counter.updateValue(0L, timestampMs);
                    this.mAccumulatedMultiStateChargeMicroCoulomb[i10] = counter;
                }
                if (counter != null) {
    ...
    public void dump(PrintWriter pw) {
        ...
        for (int index = 0; index < this.mAccumulatedChargeMicroCoulomb.length; index++) {
            ...
            if (longMultiStateCounterArr != null && (counter = longMultiStateCounterArr[index]) != null) {
                pw.print(" [");
                for (int i10 = 0; i10 < this.mConfig.mStateNames.length; i10++) {
                    if (i10 != 0) {
                        pw.print(" ");
                    }
                    pw.print(this.mConfig.mStateNames[i10]);
                    pw.print(": ");
                    pw.print(counter.getCount(i10));
                }
                pw.print("]");
            }
