...
public final class AtomicDirectory {
    ...
    public FileOutputStream openWrite(File file) throws IOException {
        if (file.isDirectory() || !file.getParentFile().equals(this.mBaseDirectory)) {
            throw new IllegalArgumentException("Must be a file in " + ((Object) this.mBaseDirectory));
        }
    ...
    public void closeWrite(FileOutputStream destination) {
        ...
        if (indexOfValue < 0) {
            throw new IllegalArgumentException("Unknown file stream " + ((Object) destination));
        }
    ...
    public void failWrite() {
        ...
        try {
            ...
            Log.e(LOG_TAG, "Failed to restore in failWrite()", e2);
        }
    }
    ...
    private void ensureBaseDirectory() throws IOException {
        ...
        if (!this.mBaseDirectory.mkdirs()) {
            throw new IOException("Failed to create directory " + ((Object) this.mBaseDirectory));
        }
    ...
    private void backup() throws IOException {
        ...
        if (!this.mBaseDirectory.renameTo(this.mBackupDirectory)) {
            throw new IOException("Failed to backup " + ((Object) this.mBaseDirectory) + " to " + ((Object) this.mBackupDirectory));
        }
        syncParentDirectory();
    }
    ...
    private void restore() throws IOException {
        ...
        if (!this.mBackupDirectory.renameTo(this.mBaseDirectory)) {
            throw new IOException("Failed to restore " + ((Object) this.mBackupDirectory) + " to " + ((Object) this.mBaseDirectory));
        }
        syncParentDirectory();
    }
    ...
    private static void syncDirectory(File directory) {
        ...
        try {
            FileDescriptor fd2 = Os.open(path, OsConstants.O_RDONLY, 0);
            try {
                try {
                    Os.fsync(fd2);
                } catch (ErrnoException e2) {
                    Log.e(LOG_TAG, "Failed to fsync " + path, e2);
                }
            } finally {
                FileUtils.closeQuietly(fd2);
            }
        } catch (ErrnoException e10) {
            Log.e(LOG_TAG, "Failed to open " + path, e10);
        }
    }
}
