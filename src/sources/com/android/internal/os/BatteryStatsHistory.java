 ...
 */
    ...
    public static class TraceDelegate {
        ...
        public boolean tracingEnabled() {
            return Trace.isTagEnabled(PlaybackStateCompat.ACTION_PREPARE_FROM_URI) || this.mShouldSetProperty;
        }

        public void traceCounter(String name, int value) {
            Trace.traceCounter(PlaybackStateCompat.ACTION_PREPARE_FROM_URI, name, value);
        ...
        public void traceInstantEvent(String track, String name) {
            Trace.instantForTrack(PlaybackStateCompat.ACTION_PREPARE_FROM_URI, track, name);
        }
    }
    ...
    private BatteryStatsHistory(Parcel historyBuffer, File systemDir, int maxHistoryFiles, int maxHistoryBufferSize, HistoryStepDetailsCalculator stepDetailsCalculator, Clock clock, TraceDelegate tracer, BatteryStatsHistory writableHistory) {
        ...
        file.listFiles(new FilenameFilter() { // from class: com.android.internal.os.BatteryStatsHistory$$ExternalSyntheticLambda0
            ...
            public final boolean accept(File file2, String str) {
                return BatteryStatsHistory.lambda$new$0(Set.this, file2, str);
            }
        });
        if (!arraySet.isEmpty()) {
    ...
    public static /* synthetic */ boolean lambda$new$0(Set dedup, File dir, String name) {
        int c4;
        int b4 = name.lastIndexOf(FILE_SUFFIX);
        if (b4 <= 0 || (c4 = ParseUtils.parseInt(name.substring(0, b4), -1)) == -1) {
            return false;
        }
        dedup.add(Integer.valueOf(c4));
        return true;
    }
    ...
    public void startNextFile() {
        ...
        try {
            ...
            Slog.e(TAG, "Could not create history file: " + ((Object) this.mActiveFile.getBaseFile()));
        }
        synchronized (this) {
            cleanupLocked();
        }
    ...
    public boolean isResetEnabled() {
        boolean z10;
        synchronized (this) {
            z10 = this.mCleanupEnabled;
        }
        return z10;
    }

    public void reset() {
        for (Integer i10 : this.mFileNumbers) {
            getFile(i10.intValue()).delete();
        }
    ...
    public Parcel getNextParcel() {
        ...
        while (this.mCurrentFileIndex < this.mFileNumbers.size() - 1) {
            ...
            int i10 = this.mCurrentFileIndex;
            this.mCurrentFileIndex = i10 + 1;
            AtomicFile file = getFile(list2.get(i10).intValue());
            if (readFileToParcel(p10, file)) {
                int bufSize = p10.readInt();
                int curPos = p10.dataPosition();
                int i11 = curPos + bufSize;
                this.mCurrentParcelEnd = i11;
                this.mCurrentParcel = p10;
                if (curPos < i11) {
                    return p10;
                }
            } else {
        ...
        if (this.mHistoryParcels != null) {
            while (this.mParcelIndex < this.mHistoryParcels.size()) {
                ...
                int i12 = this.mParcelIndex;
                this.mParcelIndex = i12 + 1;
                Parcel p11 = list3.get(i12);
                if (skipHead(p11)) {
                    int bufSize2 = p11.readInt();
                    int curPos2 = p11.dataPosition();
                    int i13 = curPos2 + bufSize2;
                    this.mCurrentParcelEnd = i13;
                    this.mCurrentParcel = p11;
                    if (curPos2 < i13) {
                        return p11;
                    }
                }
    ...
    public boolean readFileToParcel(Parcel out, AtomicFile file) {
        try {
            ...
            Slog.e(TAG, "Error reading file " + file.getBaseFile().getPath(), e2);
            return false;
        }
    }

    private boolean skipHead(Parcel p10) {
        p10.setDataPosition(0);
        int version = p10.readInt();
        if (version != 209) {
            return false;
        }
        p10.readLong();
    ...
    private void writeToParcel(Parcel out, boolean useBlobs) {
        ...
        for (int i10 = 0; i10 < this.mFileNumbers.size() - 1; i10++) {
            AtomicFile file = getFile(this.mFileNumbers.get(i10).intValue());
            byte[] raw = new byte[0];
            try {
                raw = file.readFully();
            } catch (Exception e2) {
    ...
    private void readFromParcel(Parcel in, boolean useBlobs) {
        ...
        for (int i10 = 0; i10 < count; i10++) {
            ...
            if (temp != null && temp.length != 0) {
                Parcel p10 = Parcel.obtain();
                p10.unmarshall(temp, 0, temp.length);
                p10.setDataPosition(0);
                this.mHistoryParcels.add(p10);
            }
        }
    }
    ...
    public int getHistoryUsedSize() {
        int ret = 0;
        for (int i10 = 0; i10 < this.mFileNumbers.size() - 1; i10++) {
            ret = (int) (ret + getFile(this.mFileNumbers.get(i10).intValue()).getBaseFile().length());
        }
        int ret2 = ret + this.mHistoryBuffer.dataSize();
        if (this.mHistoryParcels != null) {
            for (int i11 = 0; i11 < this.mHistoryParcels.size(); i11++) {
                ret2 += this.mHistoryParcels.get(i11).dataSize();
            }
        }
        return ret2;
    }
    ...
    public void recordDataConnectionTypeChangeEvent(long elapsedRealtimeMs, long uptimeMs, int dataConnectionType) {
        BatteryStats.HistoryItem historyItem = this.mHistoryCur;
        historyItem.states = setBitField(historyItem.states, dataConnectionType, 9, 15872);
        writeHistoryItem(elapsedRealtimeMs, uptimeMs);
    }
    ...
    private void recordTraceEvents(int code, BatteryStats.HistoryTag tag) {
        ...
        int idx = (-49153) & code;
        if ((32768 & code) != 0) {
            prefix = BadgeDrawable.DEFAULT_EXCEED_MAX_BADGE_NUMBER_SUFFIX;
        } else {
            prefix = (code & 16384) != 0 ? "-" : "";
        }
    ...
    private void recordTraceCounters(int oldval, int newval, BatteryStats.BitDescription[] descriptions) {
        ...
        for (BatteryStats.BitDescription bd2 : descriptions) {
            if ((bd2.mask & diff) != 0) {
                if (bd2.shift < 0) {
                    value = (bd2.mask & newval) != 0 ? 1 : 0;
                } else {
                    int value2 = bd2.mask;
                    value = (value2 & newval) >> bd2.shift;
                }
                this.mTracer.traceCounter("battery_stats." + bd2.name, value);
            }
        }
    }
    ...
    public void writeHistoryItem(long elapsedRealtimeMs, long uptimeMs) {
        long j10 = this.mTrackRunningHistoryElapsedRealtimeMs;
        if (j10 != 0) {
            long diffElapsedMs = elapsedRealtimeMs - j10;
    ...
    private void writeHistoryItem(long elapsedRealtimeMs, long uptimeMs, BatteryStats.HistoryItem cur) {
        ...
        if (this.mMutable) {
            ...
            int i10 = this.mMaxHistoryBufferSize;
            if (dataSize >= i10) {
                if (i10 == 0) {
                ...
                copy.setTo(cur);
                long j10 = elapsedRealtimeMs2;
                startRecordingHistory(j10, uptimeMs, false);
                writeHistoryItem(j10, uptimeMs, copy, (byte) 0);
                return;
            }
            if (dataSize == 0) {
    ...
    public void writeHistoryDelta(Parcel dest, BatteryStats.HistoryItem cur, BatteryStats.HistoryItem last) {
        ...
        if (deltaTime < 0 || deltaTime > ZipUtils.UPPER_UNIXTIME_BOUND) {
        ...
        byte b4 = cur.batteryLevel;
        byte b10 = this.mLastHistoryStepLevel;
        if (b4 < b10 || b10 == 0) {
        ...
        int i10 = cur.batteryChargeUah;
        int lastBatteryLevelInt2 = last.batteryChargeUah;
        boolean batteryChargeChanged = i10 != lastBatteryLevelInt2;
    ...
    private int buildBatteryLevelInt(BatteryStats.HistoryItem h10) {
        int bits = setBitField(0, h10.batteryLevel, 25, DELTA_STATE_MASK);
        return setBitField(setBitField(bits, h10.batteryTemperature, 15, 33521664), h10.batteryVoltage, 1, HISTORY_TAG_INDEX_LIMIT);
    }

    private int buildStateInt(BatteryStats.HistoryItem h10) {
        int plugType = 0;
        if ((h10.batteryPlugType & 1) != 0) {
            plugType = 1;
        } else if ((h10.batteryPlugType & 2) != 0) {
            plugType = 2;
        } else if ((h10.batteryPlugType & 4) != 0) {
            plugType = 3;
        }
        return ((h10.batteryStatus & 7) << 29) | ((h10.batteryHealth & 7) << 26) | ((plugType & 3) << 24) | (h10.states & 16777215);
    }
    ...
    public void writeHistory() {
        ...
        try {
            SystemClock.uptimeMillis();
            writeHistoryBuffer(p10);
            writeParcelToFileLocked(p10, this.mActiveFile);
        } finally {
            p10.recycle();
        }
    }
    ...
    private void writeParcelToFileLocked(Parcel p10, AtomicFile file) {
