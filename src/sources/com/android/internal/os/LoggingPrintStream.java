...
public abstract class LoggingPrintStream extends PrintStream {
    ...
    public LoggingPrintStream() {
        ...
        this.builder = sb2;
        this.formatter = new Formatter(sb2, (Locale) null);
    }
    ...
    @Override // java.io.PrintStream
    public synchronized PrintStream format(Locale l10, String format, Object... args) {
        if (format == null) {
    ...
    @Override // java.io.PrintStream
    public synchronized void println(String s2) {
        if (this.builder.length() == 0 && s2 != null) {
            int length = s2.length();
            int start = 0;
            while (start < length) {
                int nextBreak = s2.indexOf(10, start);
                if (nextBreak == -1) {
                    break;
                }
                log(s2.substring(start, nextBreak));
                start = nextBreak + 1;
            }
            if (start < length) {
                log(s2.substring(start));
            }
        } else {
    ...
    public synchronized PrintStream append(char c4) {
        print(c4);
        return this;
    }
