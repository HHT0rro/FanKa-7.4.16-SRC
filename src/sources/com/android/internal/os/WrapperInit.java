    ...
    public static void main(String[] args) {
        ...
        if (fdNum != 0) {
            ...
            try {
                try {
                    fd2.setInt$(fdNum);
                    DataOutputStream os = new DataOutputStream(new FileOutputStream(fd2));
        ...
        Runnable r10 = wrapperInit(targetSdkVersion, runtimeArgs);
        r10.run();
    }
        ...
        try {
            ...
            if (data[0].permitted != data[0].inheritable || data[1].permitted != data[1].inheritable) {
                ...
                try {
                    ...
                    Slog.e(TAG, "RuntimeInit: Failed capset", e2);
                    return;
                }
            }
            for (int i10 = 0; i10 < 64; i10++) {
                int dataIndex = OsConstants.CAP_TO_INDEX(i10);
                int capMask = OsConstants.CAP_TO_MASK(i10);
                if ((data[dataIndex].inheritable & capMask) != 0) {
                    try {
                        Os.prctl(OsConstants.PR_CAP_AMBIENT, OsConstants.PR_CAP_AMBIENT_RAISE, i10, 0L, 0L);
                    } catch (ErrnoException ex) {
                        Slog.e(TAG, "RuntimeInit: Failed to raise ambient capability " + i10, ex);
                    }
                }
            }
        } catch (ErrnoException e10) {
            Slog.e(TAG, "RuntimeInit: Failed capget", e10);
        }
    }
}
