...
public class BatteryStatsHistoryIterator implements Iterator<BatteryStats.HistoryItem>, AutoCloseable {
    ...
    public boolean hasNext() {
        Parcel p10 = this.mBatteryStatsHistory.getNextParcel();
        if (p10 == null) {
            close();
            return false;
        }
    ...
    public BatteryStats.HistoryItem next() {
        ...
        try {
            readHistoryDelta(p10, this.mHistoryItem);
            ...
            Slog.wtf(TAG, "Corrupted battery history", t2);
            return null;
        }
    }
    ...
    private void readHistoryDelta(Parcel src, BatteryStats.HistoryItem cur) {
        ...
        if ((cur.states2 & 131072) != 0) {
            ...
            if ((extensionFlags & 1) != 0) {
                ...
                for (int i10 = 0; i10 < consumerCount; i10++) {
                    ...
                    this.mEnergyConsumerDetails.consumers[i10] = consumer;
                }
            }
            ...
            if ((extensionFlags & 4) != 0) {
                ...
                for (int i11 = 0; i11 < cpuBracketCount; i11++) {
                    this.mCpuUsageDetails.cpuBracketDescriptions[i11] = src.readString();
                }
