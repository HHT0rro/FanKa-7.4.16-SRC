...
public class PKCS1Encoding implements AsymmetricBlockCipher {
    ...
    private byte[] encodeBlock(byte[] in, int inOff, int inLen) throws InvalidCipherTextException {
        ...
        if (this.forPrivateKey) {
            block[0] = 1;
            for (int i10 = 1; i10 != (block.length - inLen) - 1; i10++) {
                block[i10] = -1;
            }
        } else {
            this.random.nextBytes(block);
            block[0] = 2;
            for (int i11 = 1; i11 != (block.length - inLen) - 1; i11++) {
                while (block[i11] == 0) {
                    block[i11] = (byte) this.random.nextInt();
                }
            }
        }
        int i12 = block.length;
        block[(i12 - inLen) - 1] = 0;
    ...
    private static int checkPkcs1Encoding(byte[] encoded, int pLen) {
        ...
        for (int i10 = 1; i10 < plen; i10++) {
            int tmp = encoded[i10];
            int tmp2 = tmp | (tmp >> 1);
            int tmp3 = tmp2 | (tmp2 >> 2);
            correct |= ((tmp3 | (tmp3 >> 4)) & 1) - 1;
        }
        int i11 = encoded.length;
        int correct2 = correct | encoded[i11 - (pLen + 1)];
    ...
    private byte[] decodeBlockOrRandom(byte[] in, int inOff, int inLen) throws InvalidCipherTextException {
        ...
        while (true) {
            int i11 = this.pLen;
            if (i10 < i11) {
                result[i10] = (byte) ((data[(data.length - i11) + i10] & (~correct)) | (random[i10] & correct));
                i10++;
            } else {
    ...
    private byte[] decodeBlock(byte[] in, int inOff, int inLen) throws InvalidCipherTextException {
        ...
        System.arraycopy((Object) data, start, (Object) result, 0, result.length);
        return result;
    }
    ...
    private int findStart(byte type, byte[] block) throws InvalidCipherTextException {
        ...
        for (int i10 = 1; i10 != block.length; i10++) {
            byte pad = block[i10];
            boolean z10 = false;
            if ((pad == 0) & (start < 0)) {
                start = i10;
            }
            boolean z11 = (type == 1) & (start < 0);
            if (pad != -1) {
                z10 = true;
            }
            padErr |= z11 & z10;
