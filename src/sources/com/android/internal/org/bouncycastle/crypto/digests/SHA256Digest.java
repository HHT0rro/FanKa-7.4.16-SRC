...
public class SHA256Digest extends GeneralDigest implements EncodableDigest {
    ...
    private static int Ch(int x10, int y10, int z10) {
        return (x10 & y10) ^ ((~x10) & z10);
    }

    private static int Maj(int x10, int y10, int z10) {
        return (x10 & y10) | ((x10 ^ y10) & z10);
    }

    private static int Sum0(int x10) {
        return (((x10 >>> 2) | (x10 << 30)) ^ ((x10 >>> 13) | (x10 << 19))) ^ ((x10 >>> 22) | (x10 << 10));
    }

    private static int Sum1(int x10) {
        return (((x10 >>> 6) | (x10 << 26)) ^ ((x10 >>> 11) | (x10 << 21))) ^ ((x10 >>> 25) | (x10 << 7));
    }

    private static int Theta0(int x10) {
        return (((x10 >>> 7) | (x10 << 25)) ^ ((x10 >>> 18) | (x10 << 14))) ^ (x10 >>> 3);
    }

    private static int Theta1(int x10) {
    ...
    public void reset(Memoable other) {
        SHA256Digest d10 = (SHA256Digest) other;
        copyIn(d10);
    }
