...
public class V3TBSCertificateGenerator {
    ...
    public TBSCertificate generateTBSCertificate() {
        ...
        v2.add(new DERSequence(validity));
        X500Name x500Name = this.subject;
        if (x500Name != null) {
            v2.add(x500Name);
        } else {
            v2.add(new DERSequence());
        }
        v2.add(this.subjectPublicKeyInfo);
        DERBitString dERBitString = this.issuerUniqueID;
        if (dERBitString != null) {
            v2.add(new DERTaggedObject(false, 1, dERBitString));
        }
        DERBitString dERBitString2 = this.subjectUniqueID;
        if (dERBitString2 != null) {
            v2.add(new DERTaggedObject(false, 2, dERBitString2));
        }
        Extensions extensions = this.extensions;
        if (extensions != null) {
            v2.add(new DERTaggedObject(true, 3, extensions));
        }
        return TBSCertificate.getInstance(new DERSequence(v2));
    }
}
