...
public class ExtensionsGenerator {
    ...
    public void addExtension(Extension extension) {
        if (this.extensions.containsKey(extension.getExtnId())) {
            throw new IllegalArgumentException("extension " + ((Object) extension.getExtnId()) + " already added");
        }
    ...
    public void replaceExtension(Extension extension) {
        if (!this.extensions.containsKey(extension.getExtnId())) {
            throw new IllegalArgumentException("extension " + ((Object) extension.getExtnId()) + " not present");
        }
        this.extensions.put(extension.getExtnId(), extension);
    }
    ...
    public void removeExtension(ASN1ObjectIdentifier oid) {
        if (!this.extensions.containsKey(oid)) {
            throw new IllegalArgumentException("extension " + ((Object) oid) + " not present");
        }
    ...
    public Extensions generate() {
        ...
        for (int i10 = 0; i10 != this.extOrdering.size(); i10++) {
            exts[i10] = (Extension) this.extensions.get(this.extOrdering.elementAt(i10));
        }
