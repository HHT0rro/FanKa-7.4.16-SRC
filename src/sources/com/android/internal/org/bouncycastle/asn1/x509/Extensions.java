...
public class Extensions extends ASN1Object {
    ...
    private Extensions(ASN1Sequence seq) {
        ...
        while (e2.hasMoreElements()) {
            Extension ext = Extension.getInstance(e2.nextElement());
            if (this.extensions.containsKey(ext.getExtnId())) {
                throw new IllegalArgumentException("repeated extension found: " + ((Object) ext.getExtnId()));
            }
    ...
    public Extensions(Extension[] extensions) {
        this.extensions = new Hashtable();
        this.ordering = new Vector();
        for (int i10 = 0; i10 != extensions.length; i10++) {
            Extension ext = extensions[i10];
            this.ordering.addElement(ext.getExtnId());
            this.extensions.put(ext.getExtnId(), ext);
        }
    }
