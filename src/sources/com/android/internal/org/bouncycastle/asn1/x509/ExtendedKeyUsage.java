...
public class ExtendedKeyUsage extends ASN1Object {
    ...
    private ExtendedKeyUsage(ASN1Sequence seq) {
        ...
        while (e2.hasMoreElements()) {
            ASN1Encodable o10 = (ASN1Encodable) e2.nextElement();
            if (!(o10.toASN1Primitive() instanceof ASN1ObjectIdentifier)) {
                throw new IllegalArgumentException("Only ASN1ObjectIdentifiers allowed in ExtendedKeyUsage.");
            }
            this.usageTable.put(o10, o10);
        }
    }

    public ExtendedKeyUsage(KeyPurposeId[] usages) {
        ASN1EncodableVector v2 = new ASN1EncodableVector(usages.length);
        for (int i10 = 0; i10 != usages.length; i10++) {
            v2.add(usages[i10]);
            this.usageTable.put(usages[i10], usages[i10]);
        }
        this.seq = new DERSequence(v2);
    }

    public ExtendedKeyUsage(Vector usages) {
        ASN1EncodableVector v2 = new ASN1EncodableVector(usages.size());
        Enumeration e2 = usages.elements();
        while (e2.hasMoreElements()) {
            KeyPurposeId o10 = KeyPurposeId.getInstance(e2.nextElement());
            v2.add(o10);
            this.usageTable.put(o10, o10);
        }
        this.seq = new DERSequence(v2);
    ...
    public KeyPurposeId[] getUsages() {
        ...
        while (it.hasMoreElements()) {
            temp[i10] = KeyPurposeId.getInstance(it.nextElement());
            i10++;
        }
        return temp;
    }
