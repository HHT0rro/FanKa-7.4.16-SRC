...
public class ASN1StreamParser {
    ...
    public ASN1Primitive readTaggedObject(boolean constructed, int tag) throws IOException {
        ...
        ASN1EncodableVector v2 = readVector();
        if (this._in instanceof IndefiniteLengthInputStream) {
            if (v2.size() == 1) {
                return new BERTaggedObject(true, tag, v2.get(0));
            }
            return new BERTaggedObject(false, tag, BERFactory.createSequence(v2));
        }
        if (v2.size() == 1) {
            return new DLTaggedObject(true, tag, v2.get(0));
        }
        return new DLTaggedObject(false, tag, DLFactory.createSequence(v2));
    }
    ...
    public ASN1EncodableVector readVector() throws IOException {
        ...
        do {
            if (obj instanceof InMemoryRepresentable) {
                v2.add(((InMemoryRepresentable) obj).getLoadedObject());
            } else {
