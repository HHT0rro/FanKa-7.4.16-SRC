...
public class X509Extensions extends ASN1Object {
    ...
    public X509Extensions(ASN1Sequence seq) {
        ...
        while (e2.hasMoreElements()) {
            ...
            if (s2.size() == 3) {
                this.extensions.put(s2.getObjectAt(0), new X509Extension(ASN1Boolean.getInstance(s2.getObjectAt(1)), ASN1OctetString.getInstance(s2.getObjectAt(2))));
            } else if (s2.size() == 2) {
                this.extensions.put(s2.getObjectAt(0), new X509Extension(false, ASN1OctetString.getInstance(s2.getObjectAt(1))));
            } else {
    ...
    public X509Extensions(Vector ordering, Hashtable extensions) {
        ...
        if (ordering == null) {
            ...
            e2 = ordering.elements();
        }
        while (e2.hasMoreElements()) {
            this.ordering.addElement(ASN1ObjectIdentifier.getInstance(e2.nextElement()));
        }
        Enumeration e10 = this.ordering.elements();
        while (e10.hasMoreElements()) {
            ASN1ObjectIdentifier oid = ASN1ObjectIdentifier.getInstance(e10.nextElement());
    ...
    public X509Extensions(Vector objectIDs, Vector values) {
        this.extensions = new Hashtable();
        this.ordering = new Vector();
        Enumeration e2 = objectIDs.elements();
        while (e2.hasMoreElements()) {
        ...
        Enumeration e10 = this.ordering.elements();
        while (e10.hasMoreElements()) {
            ASN1ObjectIdentifier oid = (ASN1ObjectIdentifier) e10.nextElement();
    ...
    public boolean equivalent(X509Extensions other) {
        ...
        Enumeration e12 = this.extensions.keys();
        while (e12.hasMoreElements()) {
            Object key = e12.nextElement();
    ...
    private ASN1ObjectIdentifier[] getExtensionOIDs(boolean isCritical) {
        ...
        for (int i10 = 0; i10 != this.ordering.size(); i10++) {
            Object oid = this.ordering.elementAt(i10);
    ...
    private ASN1ObjectIdentifier[] toOidArray(Vector oidVec) {
        ...
        for (int i10 = 0; i10 != oids.length; i10++) {
            oids[i10] = (ASN1ObjectIdentifier) oidVec.elementAt(i10);
        }
        return oids;
    }
}
