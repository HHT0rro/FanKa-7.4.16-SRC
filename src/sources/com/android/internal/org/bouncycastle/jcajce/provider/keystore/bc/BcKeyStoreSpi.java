 ...
 */
            ...
            if (this.type == 4) {
                ...
                try {
                    ...
                    try {
                        ...
                        try {
                            k10 = BcKeyStoreSpi.this.decodeKey(new DataInputStream(cIn2));
                        } catch (Exception e10) {
                            ...
                            k10 = BcKeyStoreSpi.this.decodeKey(new DataInputStream(cIn3));
                        }
                        if (k10 == null) {
                        ...
                        DataOutputStream dOut2 = new DataOutputStream(cOut);
                        BcKeyStoreSpi.this.encodeKey(k10, dOut2);
                        dOut2.close();
                        this.obj = bOut.toByteArray();
                        return k10;
                    }
    ...
    public Key decodeKey(DataInputStream dIn) throws IOException {
        ...
        } else if (format.equals(e.f29912b) || format.equals("X509")) {
    ...
    protected Cipher makePBECipher(String algorithm, int mode, char[] password, byte[] salt, int iterationCount) throws IOException {
        try {
            ...
            throw new IOException("Error initialising store of key store: " + ((Object) e2));
        }
    }
    ...
    public String engineGetCertificateAlias(Certificate cert) {
        ...
        while (e2.hasMoreElements()) {
            StoreEntry entry = (StoreEntry) e2.nextElement();
            if (entry.getObject() instanceof Certificate) {
                Certificate c4 = (Certificate) entry.getObject();
                if (c4.equals(cert)) {
    ...
    protected void loadStore(InputStream in) throws IOException {
        ...
        for (int type = dIn.read(); type > 0; type = dIn.read()) {
            ...
            if (chainLength == 0) {
                ...
                for (int i10 = 0; i10 != chainLength; i10++) {
                    chain2[i10] = decodeCertificate(dIn);
                }
                chain = chain2;
            }
            switch (type) {
                ...
                case 3:
                case 4:
                    byte[] b4 = new byte[dIn.readInt()];
                    dIn.readFully(b4);
                    this.table.put(alias, new StoreEntry(alias, date, type, b4, chain));
                    break;
                default:
        ...
        while (e2.hasMoreElements()) {
            ...
            if (chain == null) {
                ...
                for (int i10 = 0; i10 != chain.length; i10++) {
                    encodeCertificate(chain[i10], dOut);
                }
            }
            int i11 = entry.getType();
            switch (i11) {
                case 1:
                ...
                case 3:
                case 4:
                    byte[] b4 = (byte[]) entry.getObject();
                    dOut.writeInt(b4.length);
                    dOut.write(b4);
                    break;
                default:
                    throw new IOException("Unknown object type in store.");
    ...
    public void engineStore(OutputStream stream, char[] password) throws IOException {
        ...
        for (int i10 = 0; i10 != passKey.length; i10++) {
            passKey[i10] = 0;
        }
