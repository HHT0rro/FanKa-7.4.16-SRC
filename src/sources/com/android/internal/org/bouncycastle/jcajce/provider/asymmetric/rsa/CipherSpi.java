...
public class CipherSpi extends BaseCipherSpi {
    ...
    protected int engineGetKeySize(Key key) {
        if (key instanceof RSAPrivateKey) {
            RSAPrivateKey k10 = (RSAPrivateKey) key;
            return k10.getModulus().bitLength();
        }
        if (key instanceof RSAPublicKey) {
            RSAPublicKey k11 = (RSAPublicKey) key;
            return k11.getModulus().bitLength();
        }
    ...
    public void engineSetMode(String mode) throws NoSuchAlgorithmException {
        String md2 = Strings.toUpperCase(mode);
        if (md2.equals("NONE") || md2.equals("ECB")) {
            return;
        }
        if (md2.equals("1")) {
    ...
    public int engineDoFinal(byte[] input, int inputOffset, int inputLen, byte[] output, int outputOffset) throws IllegalBlockSizeException, BadPaddingException, ShortBufferException {
        ...
        for (int i10 = 0; i10 != out.length; i10++) {
            output[outputOffset + i10] = out[i10];
        }
        int i11 = out.length;
        return i11;
    }
