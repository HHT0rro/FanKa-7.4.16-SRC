...
public class CompositePublicKey implements PublicKey {
    ...
    public byte[] getEncoded() {
        ...
        for (int i10 = 0; i10 != this.keys.size(); i10++) {
            v2.add(SubjectPublicKeyInfo.getInstance(this.keys.get(i10).getEncoded()));
        }
        try {
            return new SubjectPublicKeyInfo(new AlgorithmIdentifier(MiscObjectIdentifiers.id_alg_composite), new DERSequence(v2)).getEncoded(ASN1Encoding.DER);
    ...
    public boolean equals(Object o10) {
        ...
        if (o10 instanceof CompositePublicKey) {
            return this.keys.equals(((CompositePublicKey) o10).keys);
        }
        return false;
    }
}
