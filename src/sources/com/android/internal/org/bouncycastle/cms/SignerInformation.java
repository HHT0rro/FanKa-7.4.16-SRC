...
public class SignerInformation {
    ...
    public SignerInformation(SignerInfo info, ASN1ObjectIdentifier contentType, CMSProcessable content, byte[] resultDigest) {
        ...
        SignerIdentifier s2 = info.getSID();
        if (s2.isTagged()) {
            ASN1OctetString octs = ASN1OctetString.getInstance(s2.getId());
            this.sid = new SignerId(octs.getOctets());
        } else {
            IssuerAndSerialNumber iAnds = IssuerAndSerialNumber.getInstance(s2.getId());
    ...
    public byte[] getDigestAlgParams() {
        try {
            ...
            throw new RuntimeException("exception getting digest parameters " + ((Object) e2));
        }
    }
    ...
    public byte[] getEncryptionAlgParams() {
        try {
            ...
            throw new RuntimeException("exception getting encryption parameters " + ((Object) e2));
        }
    }
    ...
    public SignerInformationStore getCounterSignatures() {
        ...
        for (int i10 = 0; i10 < allCSAttrs.size(); i10++) {
            Attribute counterSignatureAttribute = (Attribute) allCSAttrs.get(i10);
    ...
    private boolean doVerify(SignerInformationVerifier verifier) throws CMSException {
        ...
        try {
            ...
            try {
                ...
                sigOut.close();
                ASN1Primitive validContentType = getSingleValuedSignedAttribute(CMSAttributes.contentType, e.f4632d);
                ...
                if (unsignedAttrTable2 != null) {
                    ...
                    for (int i10 = 0; i10 < csAttrs.size(); i10++) {
                        Attribute csAttr = Attribute.getInstance(csAttrs.get(i10));
                ...
                throw new CMSException("can't create digest calculator: " + e10.getMessage(), e10);
    ...
    private ASN1Primitive getSingleValuedSignedAttribute(ASN1ObjectIdentifier attrOID, String printableName) throws CMSException {
        ...
        ASN1EncodableVector v2 = signedAttrTable.getAll(attrOID);
        switch (v2.size()) {
            case 0:
                return null;
            case 1:
                Attribute t2 = (Attribute) v2.get(0);
                ASN1Set attrValues = t2.getAttrValues();
    ...
    public static SignerInformation addCounterSigners(SignerInformation signerInformation, SignerInformationStore counterSigners) {
        ...
        if (unsignedAttr != null) {
            v2 = unsignedAttr.toASN1EncodableVector();
        } else {
            v2 = new ASN1EncodableVector();
        }
        ASN1EncodableVector sigs = new ASN1EncodableVector();
        Iterator it = counterSigners.getSigners().iterator2();
        while (it.hasNext()) {
            sigs.add(it.next().toASN1Structure());
        }
        v2.add(new Attribute(CMSAttributes.counterSignature, new DERSet(sigs)));
        return new SignerInformation(new SignerInfo(sInfo.getSID(), sInfo.getDigestAlgorithm(), sInfo.getAuthenticatedAttributes(), sInfo.getDigestEncryptionAlgorithm(), sInfo.getEncryptedDigest(), new DERSet(v2)), signerInformation.contentType, signerInformation.content, null);
    }
}
