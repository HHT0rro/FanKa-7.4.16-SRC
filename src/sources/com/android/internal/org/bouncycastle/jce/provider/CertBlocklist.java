...
public class CertBlocklist {
    ...
    private static boolean isHex(String value) {
        try {
            ...
            logger.log(Level.WARNING, "Could not parse hex value " + value, (Throwable) e2);
            return false;
        }
    }
    ...
    private static String readBlocklist(String path) {
        try {
            ...
            logger.log(Level.WARNING, "Could not read blocklist", (Throwable) e10);
            return "";
        }
    }
    ...
    private static ByteArrayOutputStream readFileAsBytes(String path) throws IOException {
        RandomAccessFile f10 = null;
        try {
            f10 = new RandomAccessFile(path, t.f36226k);
            ByteArrayOutputStream bytes = new ByteArrayOutputStream((int) f10.length());
            byte[] buffer = new byte[8192];
            while (true) {
                int byteCount = f10.read(buffer);
                if (byteCount == -1) {
                    return bytes;
                }
        ...
        if (!serialBlocklist.equals("")) {
            for (String value : serialBlocklist.split(",")) {
                try {
                    ...
                    logger.log(Level.WARNING, "Tried to blocklist invalid serial number " + value, (Throwable) e2);
                }
            }
        }
