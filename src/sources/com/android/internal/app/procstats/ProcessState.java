 ...
 */
    ...
    static class PssAggr {
        ...
        void add(long newPss, long newSamples) {
            double d10 = this.pss;
            long j10 = this.samples;
            this.pss = ((long) ((d10 * j10) + (newPss * newSamples))) / (j10 + newSamples);
            this.samples = j10 + newSamples;
        }
    }
    ...
    public ProcessState(ProcessState commonProcess, String pkg, int uid, long vers, String name, long now) {
        ...
        this.mUid = uid;
        this.mVersion = vers;
        int i10 = commonProcess.mCurCombinedState;
        this.mCurCombinedState = i10;
        this.mStartTime = now;
        if (i10 != -1) {
    ...
    public ProcessState clone(long now) {
        ...
        System.arraycopy((Object) this.mTotalRunningPss, 0, (Object) pnew.mTotalRunningPss, 0, 10);
    ...
    public void add(ProcessState other) {
        ...
        int i10 = other.mNumCachedKill;
        if (i10 > 0) {
            addCachedKill(i10, other.mMinCachedKillPss, other.mAvgCachedKillPss, other.mMaxCachedKillPss);
        }
        if (other.mCommonSources != null) {
            ...
            for (int i11 = 0; i11 < size; i11++) {
                AssociationState.SourceKey key = other.mCommonSources.keyAt(i11);
                ...
                state.add(other.mCommonSources.valueAt(i11));
            }
        }
    }
    ...
    public void writeToParcel(Parcel parcel, long j10) {
        parcel.writeInt(this.mMultiPackage ? 1 : 0);
        this.mDurations.writeToParcel(parcel);
        this.mPssTable.writeToParcel(parcel);
        for (int i10 = 0; i10 < 10; i10++) {
            parcel.writeLong(this.mTotalRunningPss[i10]);
        }
        parcel.writeLong(getTotalRunningDuration(j10));
        parcel.writeInt(0);
        ...
        for (int i11 = 0; i11 < size; i11++) {
            AssociationState.SourceKey keyAt = this.mCommonSources.keyAt(i11);
            AssociationState.SourceState valueAt = this.mCommonSources.valueAt(i11);
    ...
    public boolean readFromParcel(Parcel in, int version, boolean fully) {
        ...
        for (int i10 = 0; i10 < 10; i10++) {
            this.mTotalRunningPss[i10] = in.readLong();
        }
        ...
        if (numOfSources > 0) {
            ...
            for (int i11 = 0; i11 < numOfSources; i11++) {
    ...
    public void decActiveServices(String serviceName) {
        ...
        int i10 = this.mNumActiveServices - 1;
        this.mNumActiveServices = i10;
        if (i10 < 0) {
    ...
    public void incStartedServices(int memFactor, long now, String serviceName) {
        ...
        int i10 = this.mNumStartedServices + 1;
        this.mNumStartedServices = i10;
        if (i10 == 1 && this.mCurCombinedState == -1) {
    ...
    public void decStartedServices(int memFactor, long now, String serviceName) {
        ...
        int i10 = this.mNumStartedServices - 1;
        this.mNumStartedServices = i10;
        if (i10 == 0 && this.mCurCombinedState % 16 == 9) {
    ...
    public void addPss(long pss, long uss, long rss, boolean always, int type, long duration, ArrayMap<String, ProcessStats.ProcessStateHolder> pkgList) {
        ...
        if (always || this.mLastPssState != this.mCurCombinedState || SystemClock.uptimeMillis() >= this.mLastPssTime + 30000) {
            ...
            int i10 = this.mCurCombinedState;
            if (i10 != -1) {
                this.mCommonProcess.mPssTable.mergeStats(i10, 1, pss, pss, pss, uss, uss, uss, rss, rss, rss);
    ...
    private void addCachedKill(int num, long minPss, long avgPss, long maxPss) {
        ...
        this.mAvgCachedKillPss = (long) (((this.mAvgCachedKillPss * i10) + avgPss) / (i10 + num));
        this.mNumCachedKill = i10 + num;
    }
    ...
    public void aggregatePss(ProcessStats.TotalMemoryUseCollection data, long now) {
        ...
        for (int i10 = 0; i10 < processState.mDurations.getKeyCount(); i10++) {
            int type = SparseMappingTable.getIdFromKey(processState.mDurations.getKeyAt(i10));
    ...
    public long computeProcessTimeLocked(int[] screenStates, int[] memStates, int[] procStates, long now) {
        long totalTime = 0;
        for (int i10 : screenStates) {
            for (int i11 : memStates) {
                for (int i12 : procStates) {
                    int bucket = ((i10 + i11) * 16) + i12;
    ...
    public void computeProcessData(ProcessStats.ProcessDataCollection data, long now) {
        ...
        while (is2 < data.screenStates.length) {
            int im3 = 0;
            while (im3 < data.memStates.length) {
                int ip2 = 0;
                while (ip2 < data.procStates.length) {
                    int bucket = ((data.screenStates[is2] + data.memStates[im3]) * 16) + data.procStates[ip2];
                    ...
                    if (samples <= maxRss) {
                        ...
                        if (data.numPss == 0) {
                            ...
                            double d10 = avgPss2;
                            avgPss = samples;
                            data.avgPss = (long) (((data.avgPss * data.numPss) + (d10 * avgPss)) / (data.numPss + avgPss));
    ...
    public void dumpAllStateCheckin(PrintWriter pw, long now) {
        ...
        for (int i11 = 0; i11 < this.mDurations.getKeyCount(); i11++) {
            int key = this.mDurations.getKeyAt(i11);
        ...
        if (!didCurState && (i10 = this.mCurCombinedState) != -1) {
            DumpUtils.printProcStateTagAndValue(pw, i10, now - this.mStartTime);
        }
    }
    ...
    public void dumpAllPssCheckin(PrintWriter pw) {
        ...
        for (int i10 = 0; i10 < N; i10++) {
            int key = this.mPssTable.getKeyAt(i10);
            int type = SparseMappingTable.getIdFromKey(key);
            pw.print(',');
    ...
    public String toString() {
        ...
        sb2.append("ProcessState{").append(Integer.toHexString(System.identityHashCode(this))).append(" ").append(this.mName).append("/").append(this.mUid).append(" pkg=").append(this.mPackage);
        if (this.mMultiPackage) {
            sb2.append(" (multi)");
        }
    ...
    public void dumpDebug(ProtoOutputStream proto, long fieldId, String procName, int uid, long now) {
        ...
        boolean didCurState = false;
        for (int i12 = 0; i12 < this.mDurations.getKeyCount(); i12++) {
            int key = this.mDurations.getKeyAt(i12);
            int type = SparseMappingTable.getIdFromKey(key);
            long time = this.mDurations.getValue(key);
            if (this.mCurCombinedState == type) {
                time += now - this.mStartTime;
        ...
        if (!didCurState && (i11 = this.mCurCombinedState) != -1) {
            durationByState.put(i11, now - this.mStartTime);
        }
        int i13 = 0;
        while (true) {
            j10 = 2246267895813L;
            j11 = 1112396529668L;
            if (i13 >= this.mPssTable.getKeyCount()) {
                break;
            }
            int key2 = this.mPssTable.getKeyAt(i13);
            ...
            if (durationByState.indexOfKey(type2) < 0) {
                ...
                i10 = i13;
                DumpUtils.printProcStateTagProto(proto, 1159641169921L, BinderLatencyProto.Dims.SERVICE_CLASS_NAME_AS_ENUM, 1159641169923L, type2);
        ...
        int i14 = 0;
        while (i14 < durationByState.size()) {
            long stateToken2 = proto.start(j10);
            int i15 = i14;
            DumpUtils.printProcStateTagProto(proto, 1159641169921L, BinderLatencyProto.Dims.SERVICE_CLASS_NAME_AS_ENUM, 1159641169923L, durationByState.keyAt(i14));
            proto.write(1112396529668L, durationByState.valueAt(i15));
        ...
        if (totalRunningDuration > 0) {
            ...
            proto.write(j12, totalRunningDuration);
    ...
    public void dumpStateDurationToStatsd(int atomTag, ProcessStats processStats, StatsEventOutput statsEventOutput) {
        ...
        while (i10 < size) {
            int size2 = size;
            int key = processState.mDurations.getKeyAt(i10);
            int type = SparseMappingTable.getIdFromKey(key);
