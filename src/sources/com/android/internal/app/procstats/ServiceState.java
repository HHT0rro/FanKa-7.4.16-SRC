...
public final class ServiceState {
    ...
    public void clearCurrentOwner(Object owner, boolean silently) {
        if (this.mOwner == owner) {
            ...
            if (this.mStarted || this.mBoundState != -1 || this.mExecState != -1 || this.mForegroundState != -1) {
                ...
                if (this.mStarted) {
                    if (!silently) {
                        Slog.wtfStack("ProcessStats", "Service owner " + owner + " cleared while started: pkg=" + this.mPackage + " service=" + this.mName + " proc=" + ((Object) this.mProc));
                    }
                    setStarted(false, 0, now);
                }
                if (this.mBoundState != -1) {
                    if (!silently) {
                        Slog.wtfStack("ProcessStats", "Service owner " + owner + " cleared while bound: pkg=" + this.mPackage + " service=" + this.mName + " proc=" + ((Object) this.mProc));
                    }
                    setBound(false, 0, now);
                }
                if (this.mExecState != -1) {
                    if (!silently) {
                        Slog.wtfStack("ProcessStats", "Service owner " + owner + " cleared while exec: pkg=" + this.mPackage + " service=" + this.mName + " proc=" + ((Object) this.mProc));
                    }
                    setExecuting(false, 0, now);
                }
                if (this.mForegroundState != -1) {
                    if (!silently) {
                        Slog.wtfStack("ProcessStats", "Service owner " + owner + " cleared while foreground: pkg=" + this.mPackage + " service=" + this.mName + " proc=" + ((Object) this.mProc));
                    }
                    setForeground(false, 0, now);
                }
            }
    ...
    public void commitStateTime(long now) {
        ...
        if (i10 != -1) {
            this.mDurations.addDuration((i10 * 5) + 0, now - this.mRunStartTime);
            this.mRunStartTime = now;
        }
        int i11 = this.mStartedState;
        if (i11 != -1) {
            this.mDurations.addDuration((i11 * 5) + 1, now - this.mStartedStartTime);
            this.mStartedStartTime = now;
        }
        int i12 = this.mBoundState;
        if (i12 != -1) {
            this.mDurations.addDuration((i12 * 5) + 2, now - this.mBoundStartTime);
            this.mBoundStartTime = now;
        }
        int i13 = this.mExecState;
        if (i13 != -1) {
            this.mDurations.addDuration((i13 * 5) + 3, now - this.mExecStartTime);
            this.mExecStartTime = now;
        }
        int i14 = this.mForegroundState;
        if (i14 != -1) {
            this.mDurations.addDuration((i14 * 5) + 4, now - this.mForegroundStartTime);
            this.mForegroundStartTime = now;
        }
    }
    ...
    public void setStarted(boolean started, int memFactor, long now) {
        if (this.mOwner == null) {
            Slog.wtf("ProcessStats", "Starting service " + ((Object) this) + " without owner");
        }
    ...
    public void updateStartedState(int memFactor, long now) {
        int i10 = this.mStartedState;
        boolean wasStarted = i10 != -1;
        boolean started = this.mStarted || this.mRestarting;
        int state = started ? memFactor : -1;
        if (i10 != state) {
            if (i10 != -1) {
                this.mDurations.addDuration((i10 * 5) + 1, now - this.mStartedStartTime);
            } else if (started) {
                this.mStartedCount++;
            }
    ...
    public void setBound(boolean bound, int memFactor, long now) {
        if (this.mOwner == null) {
            Slog.wtf("ProcessStats", "Binding service " + ((Object) this) + " without owner");
        }
        int state = bound ? memFactor : -1;
        int i10 = this.mBoundState;
        if (i10 != state) {
            if (i10 != -1) {
                this.mDurations.addDuration((i10 * 5) + 2, now - this.mBoundStartTime);
            } else if (bound) {
                this.mBoundCount++;
            }
    ...
    public void setExecuting(boolean executing, int memFactor, long now) {
        if (this.mOwner == null) {
            Slog.wtf("ProcessStats", "Executing service " + ((Object) this) + " without owner");
        }
        int state = executing ? memFactor : -1;
        int i10 = this.mExecState;
        if (i10 != state) {
            if (i10 != -1) {
                this.mDurations.addDuration((i10 * 5) + 3, now - this.mExecStartTime);
            } else if (executing) {
                this.mExecCount++;
            }
    ...
    public void setForeground(boolean foreground, int memFactor, long now) {
        if (this.mOwner == null) {
            Slog.wtf("ProcessStats", "Foregrounding service " + ((Object) this) + " without owner");
        }
        int state = foreground ? memFactor : -1;
        int i10 = this.mForegroundState;
        if (i10 != state) {
            if (i10 != -1) {
                this.mDurations.addDuration((i10 * 5) + 4, now - this.mForegroundStartTime);
            } else if (foreground) {
    ...
    private void dumpTimeCheckin(PrintWriter pw, String label, String packageName, int uid, long vers, String serviceName, int serviceType, int opCount, int curState, long curStartTime, long now) {
        ...
        int i10 = 0;
        while (i10 < N) {
            int key = serviceState.mDurations.getKeyAt(i10);
    ...
    public void writeTypeToProto(ProtoOutputStream proto, long fieldId, int opType, int serviceType, int opCount, int curState, long curStartTime, long now) {
        ...
        for (int i10 = 0; i10 < N; i10++) {
            ...
            if (type2 == serviceType) {
                ...
                DumpUtils.printProcStateAdjTagProto(proto, 1159641169921L, BinderLatencyProto.Dims.SERVICE_CLASS_NAME_AS_ENUM, type2);
        ...
        if (!didCurState && curState != -1) {
            ...
            DumpUtils.printProcStateAdjTagProto(proto, 1159641169921L, BinderLatencyProto.Dims.SERVICE_CLASS_NAME_AS_ENUM, curState);
    ...
    public String toString() {
        return "ServiceState{" + Integer.toHexString(System.identityHashCode(this)) + " " + this.mName + " pkg=" + this.mPackage + " proc=" + Integer.toHexString(System.identityHashCode(this.mProc)) + i.f4738d;
    }
}
