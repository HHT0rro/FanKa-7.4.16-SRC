...
public class AppPredictionServiceResolverComparator extends AbstractResolverComparator {
    ...
    public AppPredictionServiceResolverComparator(Context context, Intent intent, String referrerPackage, AppPredictor appPredictor, UserHandle user, ChooserActivityLogger chooserActivityLogger) {
        super(context, intent, Lists.newArrayList(new UserHandle[]{user}));
 ...
 */
    ...
    public static class ModelBuilder {
        ...
        private Map<ComponentName, Integer> buildTargetRanksMapFromSortedTargets(List<AppTarget> sortedAppTargets) {
            ...
            for (int i10 = 0; i10 < sortedAppTargets.size(); i10++) {
                ComponentName componentName = new ComponentName(sortedAppTargets.get(i10).getPackageName(), sortedAppTargets.get(i10).getClassName());
                targetRanks.put(componentName, Integer.valueOf(i10));
                Log.i(AppPredictionServiceResolverComparator.TAG, "handleSortedAppTargets, sortedAppTargets #" + i10 + ": " + ((Object) componentName));
            }
            return targetRanks;
        }
