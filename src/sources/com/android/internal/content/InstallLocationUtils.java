 ...
 */
            ...
            if (sDefaultTestableInterface == null) {
                sDefaultTestableInterface = new TestableInterface() { // from class: com.android.internal.content.InstallLocationUtils.1
                    ...
                    public boolean getAllow3rdPartyOnInternalConfig(Context context) {
                        return context.getResources().getBoolean(17891348);
                    }
    ...
    public static String resolveInstallVolume(Context context, PackageInstaller.SessionParams params, TestableInterface testInterface) throws IOException {
        ...
        if (!volumePaths.isEmpty() && ZipUtils.UPPER_UNIXTIME_BOUND == params.sizeBytes && SystemProperties.getBoolean("debug.pm.install_skip_size_check_for_maxint", false)) {
            return bestCandidate;
        }
    ...
    public static long calculateInstalledSize(PackageLite pkg, String abiOverride, FileDescriptor fd2) throws IOException {
        NativeLibraryHelper.Handle handle = null;
        try {
            handle = fd2 != null ? NativeLibraryHelper.Handle.createFd(pkg, fd2) : NativeLibraryHelper.Handle.create(pkg);
