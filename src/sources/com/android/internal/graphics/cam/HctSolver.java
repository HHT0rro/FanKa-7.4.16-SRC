...
public class HctSolver {
    ...
    static double hueOf(double[] linrgb) {
        ...
        double a10 = (((rA * 11.0d) + ((-12.0d) * gA)) + bA) / 11.0d;
        double b4 = ((rA + gA) - (2.0d * bA)) / 9.0d;
        return Math.atan2(b4, a10);
    }

    static boolean areInCyclicOrder(double a10, double b4, double c4) {
        double deltaAB = sanitizeRadians(b4 - a10);
        double deltaAC = sanitizeRadians(c4 - a10);
    ...
    static double[] lerpPoint(double[] source, double t2, double[] target) {
        return new double[]{source[0] + ((target[0] - source[0]) * t2), source[1] + ((target[1] - source[1]) * t2), source[2] + ((target[2] - source[2]) * t2)};
    }

    static double[] setCoordinate(double[] source, double coordinate, double[] target, int axis) {
        double t2 = intercept(source[axis], coordinate, target[axis]);
        return lerpPoint(source, t2, target);
    }

    static boolean isBounded(double x10) {
        return ShadowDrawableWrapper.COS_45 <= x10 && x10 <= 100.0d;
    }

    static double[] nthVertex(double y10, int n10) {
        double[] dArr = Y_FROM_LINRGB;
        double kR = dArr[0];
        double kG = dArr[1];
        double kB = dArr[2];
        int i10 = n10 % 4;
        double coordB = ShadowDrawableWrapper.COS_45;
        double coordA = i10 <= 1 ? 0.0d : 100.0d;
        if (n10 % 2 != 0) {
            coordB = 100.0d;
        }
        if (n10 >= 4) {
            if (n10 < 8) {
                double b4 = coordA;
                double r10 = coordB;
                double g3 = ((y10 - (r10 * kR)) - (b4 * kB)) / kG;
                if (isBounded(g3)) {
                    return new double[]{r10, g3, b4};
                }
                return new double[]{-1.0d, -1.0d, -1.0d};
            }
            double b10 = coordA;
            double g10 = coordB;
            double b11 = ((y10 - (b10 * kR)) - (g10 * kG)) / kB;
            if (isBounded(b11)) {
                return new double[]{b10, g10, b11};
            }
            return new double[]{-1.0d, -1.0d, -1.0d};
        }
        double g11 = coordA;
        double b12 = coordB;
        double r11 = ((y10 - (g11 * kG)) - (b12 * kB)) / kR;
        if (isBounded(r11)) {
            return new double[]{r11, g11, b12};
        }
        return new double[]{-1.0d, -1.0d, -1.0d};
    }

    static double[][] bisectToSegment(double y10, double targetHue) {
        ...
        for (int n10 = 0; n10 < 12; n10++) {
            double[] mid = nthVertex(y10, n10);
            if (mid[0] >= ShadowDrawableWrapper.COS_45) {
    ...
    static int criticalPlaneBelow(double x10) {
        return (int) Math.floor(x10 - 0.5d);
    }

    static int criticalPlaneAbove(double x10) {
        return (int) Math.ceil(x10 - 0.5d);
    }

    static int bisectToLimit(double y10, double targetHue) {
        int lPlane;
        int rPlane;
        double[][] segment = bisectToSegment(y10, targetHue);
        ...
        for (int axis = 0; axis < 3; axis++) {
            if (left[axis] != right[axis]) {
                ...
                int rPlane2 = rPlane;
                int i10 = 0;
                while (i10 < 8 && Math.abs(rPlane2 - lPlane) > 1) {
    ...
    static double inverseChromaticAdaptation(double adapted) {
        ...
        double base = Math.max(ShadowDrawableWrapper.COS_45, (27.13d * adaptedAbs) / (400.0d - adaptedAbs));
        return CamUtils.signum(adapted) * Math.pow(base, 2.380952380952381d);
    }

    static int findResultByJ(double hueRadians, double chroma, double y10) {
        double j10 = Math.sqrt(y10) * 11.0d;
        ...
        double p12 = 3846.153846153846d * eHue * viewingConditions.getNc() * viewingConditions.getNcb();
        double hSin = Math.sin(hueRadians);
        double hCos = Math.cos(hueRadians);
        int iterationRound = 0;
        while (iterationRound < 5) {
            double jNormalized = j10 / 100.0d;
            double alpha = (chroma == ShadowDrawableWrapper.COS_45 || j10 == ShadowDrawableWrapper.COS_45) ? 0.0d : chroma / Math.sqrt(jNormalized);
            Frame viewingConditions2 = viewingConditions;
            double tInnerCoeff2 = tInnerCoeff;
            double t2 = Math.pow(alpha * tInnerCoeff, 1.1111111111111112d);
            double eHue2 = eHue;
            double acExponent = (1.0d / viewingConditions2.getC()) / viewingConditions2.getZ();
            double ac2 = viewingConditions2.getAw() * Math.pow(jNormalized, acExponent);
            double p22 = ac2 / viewingConditions2.getNbb();
            double gamma = (((p22 + 0.305d) * 23.0d) * t2) / (((23.0d * p12) + ((t2 * 11.0d) * hCos)) + ((108.0d * t2) * hSin));
            double a10 = gamma * hCos;
            double b4 = gamma * hSin;
            double rA = (((p22 * 460.0d) + (451.0d * a10)) + (288.0d * b4)) / 1403.0d;
            double gA = (((p22 * 460.0d) - (891.0d * a10)) - (261.0d * b4)) / 1403.0d;
            double bA = (((460.0d * p22) - (220.0d * a10)) - (6300.0d * b4)) / 1403.0d;
            ...
            if (linrgbR < ShadowDrawableWrapper.COS_45 || linrgbG < ShadowDrawableWrapper.COS_45) {
                return 0;
            }
            if (linrgbB < ShadowDrawableWrapper.COS_45) {
                return 0;
            }
            double[] dArr = Y_FROM_LINRGB;
            double kR = dArr[0];
    ...
    public static int solveToInt(double hueDegrees, double chroma, double lstar) {
        ...
        double y10 = CamUtils.yFromLstar(lstar);
        int exactAnswer = findResultByJ(hueRadians, chroma, y10);
        if (exactAnswer != 0) {
            return exactAnswer;
        }
        return bisectToLimit(y10, hueRadians);
    }
